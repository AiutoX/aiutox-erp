...
// Fetches a comment's direct replies (GET /{id}/thread)
...
// Fetches a comment's revision history (GET /{id}/revisions, CMT-008).
// Callers must pass `enabled` gated on comments.manage — this hook does not
// check permissions itself, it only avoids firing when commentId is empty.
export function useCommentRevisions(
  commentId: string,
  options?: { enabled?: boolean }
) {
  return useQuery({
    queryKey: ["comments", commentId, "revisions"],
...
export function useCreateCommentWithAttachment() {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: ({ payload, file }: { payload: CommentCreate; file: File }) =>
      createCommentWithAttachment(payload, file),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["comments"] });
    },
    onError: (error) => {
      console.error("Failed to create comment with attachment:", error);
    },
  });
}
