...
export async function listAutomationRules(
  ...
  const response = await apiClient.get<StandardListResponse<AutomationRule>>(
    `/automation/rules?${searchParams.toString()}`
  );
...
export async function listAutomationExecutions(
  ...
  >(`/automation/rules/${id}/executions?${searchParams.toString()}`);
  return response.data;
}
...
> {
  const response = await apiClient.get<StandardResponse<NodeCatalogItem[]>>(
    "/automation/meta/data-sources"
  );
  return response.data;
}

export interface ConditionOperatorItem {
  operator: string;
  description: string;
  value_types: string[];
  negatable: boolean;
...
/**
 * Get available condition operators (canonical vocabulary — mirrors the
 * backend evaluator's actual accepted operators)
 */
export async function getConditionOperators(): Promise<
  StandardResponse<ConditionOperatorItem[]>
> {
  const response = await apiClient.get<StandardResponse<ConditionOperatorItem[]>>(
...
export async function validateAutomationRule(
  ...
  >("/automation/rules/validate", payload);
  return response.data;
}
...
export async function generateWebhookSecret(
  ...
  const response = await apiClient.post<StandardResponse<WebhookSecretResponse>>(
    `/automation/rules/${id}/webhook-secret`
  );
  return response.data;
}
...
export const agentRunApi = {
  ...
  list: async (params?: {
    ...
    const res = await apiClient.get<StandardListResponse<AgentRunRead>>(
      `/ai/agent-runs?${searchParams.toString()}`
    );
    return res.data;
  },
