    ...
    (date: Date) => {
      ...
      return (
        tasks?.data?.filter((task: Task) => {
          ...
          // due_date is always a full ISO datetime string from the backend
          // (e.g. "2026-07-22T00:00:00Z"), never a bare "yyyy-MM-dd" — a
          // direct string comparison against dateStr can never match.
          const taskDateStr = formatDueDate(
            task.due_date,
        ...
        <CardContent>
          <div className="space-y-4">
            {dates.map((date) => {
              ...
              const hasOverdue =
                isPast(date) && !isToday(date) && dayTasks.length > 0;
                  ...
                  {dayTasks.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {dayTasks.map((task: Task) => (
                        ...
                        >
                          <div
                            className={`w-px h-4 rounded-full ${getPriorityColor(task.priority)}`}
                          />
