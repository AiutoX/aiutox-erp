...
from app.core.integrations.channels.telegram_receiver import router as _telegram_router
from app.core.integrations.channels.whatsapp_receiver import (
...
async def list_webhooks(
    ...
    integrations: dict = (
        integrations_result
        if isinstance(integrations_result, dict)
        else {"items": integrations_result, "total": len(integrations_result)}
...
async def update_webhook(
    ...
    integration = service.update_integration(
        ...
        status=IntegrationStatus(webhook_data.status) if webhook_data.status else None,
        user_id=current_user.id,
    )
    if not integration:
        raise_not_found("Webhook", str(webhook_id))
