...
async def create_template_category(
    ...
    category = service.create_template_category(  # type: ignore[attr-defined]
...
async def list_template_categories(
    ...
    skip = (page - 1) * page_size
    categories = service.get_template_categories(  # type: ignore[attr-defined]
        tenant_id=current_user.tenant_id,
        skip=skip,
        limit=page_size,
    )
...
async def get_template_category(
    ...
    category = service.get_template_category(category_id, current_user.tenant_id)  # type: ignore[attr-defined]
    if not category:
        raise APIException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_template_category(
    ...
    category = service.update_template_category(  # type: ignore[attr-defined]
...
async def delete_template_category(
    ...
    success = service.delete_template_category(category_id, current_user.tenant_id)  # type: ignore[attr-defined]
    if not success:
        raise APIException(
            status_code=status.HTTP_404_NOT_FOUND,
            code="TEMPLATE_CATEGORY_NOT_FOUND",
