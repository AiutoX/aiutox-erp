...
class AutomationService:
    ...
    def get_all_rules(
        ...
        Args:
            ...
            enabled: Explicit enabled-state filter (takes precedence)

        Returns:
            List of rules
        """
        return self.repository.get_all_rules(
            tenant_id, enabled_only, skip, limit, enabled=enabled
        )
    ...
    def count_all_rules(
        ...
        Returns:
            Count of rules
        """
        return self.repository.count_all_rules(tenant_id, enabled_only, enabled=enabled)
    ...
    def clone_rule(
        ...
        BUG-002 decision D4: the clone starts disabled so a copy of an active
        rule cannot fire duplicate actions immediately, and
        trigger.params.secret_hash is stripped so each rule keeps a unique
        webhook secret.

        Returns:
            The new rule, or None if the source rule was not found.
        """
        source = self.repository.get_rule_by_id(rule_id, tenant_id)
        if not source:
            return None

        trigger = dict(source.trigger) if isinstance(source.trigger, dict) else {}
        if isinstance(trigger.get("params"), dict):
            params = dict(trigger["params"])
            params.pop("secret_hash", None)
        ...
        return self.create_rule(
            ...
            trigger=trigger,
            conditions=(
                list(source.conditions) if isinstance(source.conditions, list) else None
            ),
            actions=list(source.actions) if isinstance(source.actions, list) else [],
            enabled=False,
            owner_user_id=owner_user_id,
        )
    ...
    def get_execution(
        ...
        """Get a single execution, tenant-scoped through its rule."""
        return self.repository.get_execution_by_id(execution_id, tenant_id)

    def get_stats(self, tenant_id: UUID) -> dict[str, Any]:
        """Aggregate tenant automation statistics (frontend AutomationStats)."""
        by_status = self.repository.count_executions_by_status(tenant_id)
        ...
        for (
            ...
            count,
            successes,
        ) in self.repository.top_rules_by_executions(tenant_id):
            ...
            top_rules.append(
                {
                    ...
                    "success_rate": (
                        int(successes or 0) / executions if executions else 0.0
                    ),
                }
            )

        return {
            "total_rules": self.repository.count_all_rules(tenant_id),
            "active_rules": self.repository.count_all_rules(tenant_id, enabled=True),
            "total_executions": total_executions,
            "successful_executions": by_status.get("success", 0),
            "failed_executions": by_status.get("failed", 0),
            "pending_executions": by_status.get("pending", 0),
            "executions_by_status": by_status,
            "executions_by_day": [
                {"date": day, "count": count}
                for day, count in self.repository.count_executions_by_day(tenant_id)
            ],
            "top_rules": top_rules,
        }
    ...
    async def test_rule(
        ...
        try:
            synthetic_event = Event(
                ...
                entity_type=(
                    event_type.split(".")[0] if "." in event_type else "unknown"
                ),
