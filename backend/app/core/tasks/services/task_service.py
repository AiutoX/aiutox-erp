...
class TaskService:
    ...
    def __init__(
        ...
        central_event_publisher: EventPublisher | None = None,
    ):
        ...
        # Central EventPublisher (REDIS_STREAM_DOMAIN) — separate from the legacy
        # TaskEventPublisher above, which writes to isolated "task_events"/
        # "tenant_{id}_tasks" streams that nothing consumes. Notification,
        # automation, and gamification consumers all subscribe to the domain
        # stream, so lifecycle events must also be published here to reach them.
        self.central_event_publisher: EventPublisher = (
            central_event_publisher or get_event_publisher()
        )

    async def _publish_domain_event(self, **kwargs) -> None:
        """Publish to the central domain stream, tolerating publish failures.

        The task write this follows has already been committed to the DB by
        the time this runs — a transient Redis outage must not turn an
        already-successful create/update/delete into a user-facing error for
        every downstream consumer (notifications, automation, gamification,
        calendar sync) that reads this stream.
        """
        try:
            await self.central_event_publisher.publish(**kwargs)
        except PublishError as e:
            logger.error(
                f"Failed to publish '{kwargs.get('event_type')}' to the domain "
                f"stream for task {kwargs.get('entity_id')}: {e}",
                exc_info=True,
            )
    ...
    async def create_task(
        ...
        **kwargs,
        ...
        # JSONB columns can't serialize UUID objects directly
        if kwargs.get("tag_ids") is not None:
            kwargs["tag_ids"] = [str(tag_id) for tag_id in kwargs["tag_ids"]]
        ...
        # Also publish on the central domain stream so notification/automation/
        # gamification consumers receive it.
        await self._publish_domain_event(
            event_type="task.created",
            entity_type="task",
            entity_id=UUID(str(task.id)),
            tenant_id=UUID(str(task.tenant_id)),
            user_id=UUID(str(task.created_by_id)) if task.created_by_id else None,
        ...
        **updates,
        ...
        if "status" in updates:
            ...
            if new_status == TaskStatusEnum.DONE and task.completed_at is None:
                ...
                newly_completed = previous_status != TaskStatusEnum.DONE
        ...
        if "tag_ids" in updates and updates["tag_ids"] is not None:
            updates["tag_ids"] = [str(tag_id) for tag_id in updates["tag_ids"]]
        ...
        # Also publish on the central domain stream so cross-module consumers
        # (e.g. calendar sync) receive schedule changes, not just creation.
        if task is not None:
            await self._publish_domain_event(
        ...
        if newly_completed and task is not None:
            await self._publish_domain_event(
    ...
    def get_checklist_item(self, item_id: UUID, tenant_id: UUID):
        """Get a checklist item by ID."""
        return self.repository.get_checklist_item_by_id(item_id, tenant_id)
