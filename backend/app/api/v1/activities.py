...
from app.core.auth.dependencies import require_any_permission, require_permission
...
@router.post(
    ...
    description="Create a new activity. Requires activities.manage or activities.create permission.",
)
async def create_activity(
    activity_data: ActivityCreate,
    current_user: Annotated[
        User,
        Depends(require_any_permission("activities.manage", "activities.create")),
    ],
...
async def list_activities(
    ...
    activity_type: str | None = Query(
        None, description="Filter by a single activity type"
    ),
    activity_types: str | None = Query(
        None,
        description="Filter by multiple activity types (comma-separated). "
        "Takes precedence over activity_type when both are given.",
    ),
    entity_type: str | None = Query(None, description="Filter by entity type"),
    entity_id: UUID | None = Query(None, description="Filter by entity ID"),
    search: str | None = Query(None, description="Search in title and description"),
    date_from: datetime | None = Query(
        None, description="Only include activities created on/after this datetime"
    ),
    date_to: datetime | None = Query(
        None, description="Only include activities created on/before this datetime"
    ),
) -> StandardListResponse[ActivityResponse]:
    """List activities."""
    skip = (page - 1) * page_size
    parsed_activity_types = (
        [t for t in activity_types.split(",") if t] if activity_types else None
    )
...
@router.put(
    ...
    description="Update an activity. Requires activities.manage or activities.edit permission.",
)
async def update_activity(
    activity_data: ActivityUpdate,
    current_user: Annotated[
        User, Depends(require_any_permission("activities.manage", "activities.edit"))
    ],
    service: Annotated[ActivityService, Depends(get_activity_service)],
    activity_id: UUID = Path(..., description="Activity ID"),
) -> StandardResponse[ActivityResponse]:
...
@router.delete(
    ...
    description="Delete an activity. Requires activities.manage or activities.delete permission.",
)
async def delete_activity(
    current_user: Annotated[
        User,
        Depends(require_any_permission("activities.manage", "activities.delete")),
    ],
    service: Annotated[ActivityService, Depends(get_activity_service)],
    activity_id: UUID = Path(..., description="Activity ID"),
) -> None:
