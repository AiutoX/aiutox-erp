...
> {
  const response = await apiClient.get("/tasks/status-definitions");
...
export async function updateStatusDefinition(
  ...
  const response = await apiClient.put(
    `/tasks/status-definitions/${statusId}`,
    data
  );
  return response.data;
}
...
export async function deleteStatusDefinition(statusId: string): Promise<void> {
  await apiClient.delete(`/tasks/status-definitions/${statusId}`);
}
...
export async function reorderStatusDefinitions(
  ...
  const response = await apiClient.post(
    `/tasks/status-definitions/reorder?${params.toString()}`
  );
  return response.data;
}
