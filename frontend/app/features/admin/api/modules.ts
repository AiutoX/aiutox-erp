...
export const modulesApi = {
  ...
  async installModule(
    ...
    const response = await apiClient.post<{
      ...
      tier: string;
      install_order: string[];
    }>(`/admin/modules/${moduleId}/install`, {
      version,
      tier,
    });
  ...
  async disableModule(moduleId: string): Promise<{
    ...
    const response = await apiClient.post<{
      module: string;
      state: string;
    }>(`/admin/modules/${moduleId}/disable`);
    return response.data;
  },
  ...
  async uninstallRequestModule(moduleId: string): Promise<{
    ...
    const response = await apiClient.post<{
      ...
      state: string;
      grace_deadline: string | null;
    }>(`/admin/modules/${moduleId}/uninstall-request`);
    return response.data;
  },
  ...
  async reactivateModule(moduleId: string): Promise<{
    ...
    const response = await apiClient.post<{
      module: string;
      state: string;
    }>(`/admin/modules/${moduleId}/reactivate`);
    return response.data;
  },
  ...
  async hardUninstallModule(moduleId: string): Promise<{
    ...
    const response = await apiClient.post<{
      ...
      state: string;
      deleted_records: Record<string, number>;
    }>(`/admin/modules/${moduleId}/hard-uninstall`);
    return response.data;
  },
};
