...
from app.core.files.models import Folder
...
@router.post(
    ...
    description="Create a new folder. Requires files.edit permission.",
)
    ...
    def build_tree_item(folder: Folder) -> FolderTreeItem:
        ...
        try:
            ...
            folder_item = FolderTreeItem(
                ...
                tenant_id=UUID(str(folder.tenant_id)),
                name=folder.name,
                ...
                entity_id=UUID(str(folder.entity_id)) if folder.entity_id else None,
                metadata=(
                    folder.folder_metadata
                    if isinstance(folder.folder_metadata, dict)
                    else None
                ),
                created_by=UUID(str(folder.created_by)) if folder.created_by else None,
...
@router.put(
    ...
    description="Update a folder. Requires files.edit permission.",
)
...
@router.delete(
    ...
    description="Delete a folder. Requires files.delete permission.",
)
...
@router.post(
    ...
    response_model=StandardResponse[FolderResponse],
    summary="Move folder",
    description="Move a folder to a new parent. Requires files.edit permission.",
)
...
@router.post(
    ...
    description="Move files and folders to a target folder. Requires files.edit permission.",
)
async def move_items(
    ...
    for file_id in move_data.file_ids:
        ...
        if file:
            file.folder_id = move_data.target_folder_id  # type: ignore[assignment]
            service.db.commit()
...
async def update_folder_permissions(
    ...
    if folder.created_by != current_user.id:
        ...
        if not current_user.has_permission("folders.manage_users"):  # type: ignore[attr-defined]
            raise APIException(
                status_code=status.HTTP_403_FORBIDDEN,
