...
class EventConsumer:
    ...
    async def _move_to_failed_stream(
        ...
        await redis_client.xadd(self.settings.REDIS_STREAM_FAILED, failed_data)  # type: ignore[arg-type, misc]
        logger.warning(
...
# Used by cross-module consumers that register handlers at startup before the
# async event bus is running (e.g. properties.events.consumers).
# ---------------------------------------------------------------------------

_subscriptions: dict[str, list[Callable[..., Awaitable[None]]]] = {}


def subscribe(topic: str, handler: Callable[..., Awaitable[None]]) -> None:
    """Register an async handler for a topic.

    Handlers are stored in a module-level registry and wired into the
    RedisEventBus by the application startup routine.
    """
    if topic not in _subscriptions:
        _subscriptions[topic] = []
    _subscriptions[topic].append(handler)
    logger.debug(f"Registered handler for topic '{topic}'")


def get_subscriptions() -> dict[str, list[Callable[..., Awaitable[None]]]]:
    """Return the current subscription registry."""
