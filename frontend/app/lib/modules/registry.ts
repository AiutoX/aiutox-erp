...
const MODULE_GROUP_MAP: Record<string, { groupId: string; groupName: string }> =
  {
    ...
    // Inmobiliario
    leases: { groupId: "real-estate", groupName: "Inmobiliario" },
    maintenance: { groupId: "real-estate", groupName: "Inmobiliario" },
    properties: { groupId: "real-estate", groupName: "Inmobiliario" },
  };
...
class ModuleRegistry {
  ...
  private inferContextualIcon(
    ...
    if (semanticText.includes("webhook") || semanticText.includes("hook")) {
      return WebhookIcon;
    }
    if (semanticText.includes("integr")) {
      return GlobeIcon;
    }
    if (
      semanticText.includes("proveedor ia") ||
      semanticText.includes("ai-provider") ||
      semanticText.includes("ai_provider")
    ) {
      return FlashIcon;
    }
    if (
      semanticText.includes("tenant") ||
      semanticText.includes("organizaci")
    ) {
      return Building06Icon;
    }
    if (
      semanticText.includes("gestión de módulos") ||
      semanticText.includes("gestion de modulos") ||
      semanticText.includes("modules")
    ) {
      return PuzzleIcon;
    }
    if (
      semanticText.includes("tema") ||
      semanticText.includes("branding") ||
      semanticText.includes("theme")
    ) {
      return SwatchIcon;
    }
    if (
      semanticText.includes("acciones rápidas") ||
      semanticText.includes("acciones rapidas") ||
      semanticText.includes("quick-actions") ||
      semanticText.includes("quick_actions")
    ) {
      return Target01Icon;
    }
    ...
    if (
      semanticText.includes("manten") ||
      semanticText.includes("maintenance") ||
      semanticText.includes("cmms")
    ) {
      return Wrench01Icon;
    }
    if (
    ...
    if (
      semanticText.includes("gamification/manager") ||
      semanticText.includes("gamification.manager") ||
      semanticText.includes("administrador")
    ) {
  ...
  private toNavigationItem(
    ...
    return {
      ...
      label: dto.label,
      labelKey: dto.label_key ?? undefined,
      to: dto.path,
      icon: iconComponent,
      iconToken: dto.icon,
  ...
  async discoverModules(): Promise<FrontendModule[]> {
    ...
    try {
      ...
      for (const moduleItem of moduleList) {
        try {
          // moduleItem already carries everything discoverModules() needs
          // (navigation_items, settings_links, etc.) — the per-module detail
          // endpoint only adds has_router/model_count, which nothing reads.
  ...
  private getCategoryForModule(
    ...
    // Module ID -> category, independent of the backend's core/business
    // `type` split. The backend marks several infra modules (tasks, files,
    // calendar, import_export, integrations, gamification) as type "core"
    // alongside auth/users/config, so category lookup must not branch on
    // `type` first — every module ID is checked against one combined map.
    const categoryMap: Record<string, string> = {
      ...
      // Inmobiliario
      leases: "Inmobiliario",
      maintenance: "Inmobiliario",
      properties: "Inmobiliario",
      real_estate: "Inmobiliario", // backend module package groups the 3 above
      // Data Collection
      data_collection: "Recolección de Datos",
    };

    return categoryMap[moduleInfo.id] ?? "Sistema";
  }
