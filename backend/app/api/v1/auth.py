...
from app.schemas.audit import (
    ...
    AuditPurgeResponse,
    AuditRetentionConfigResponse,
    AuditRetentionConfigUpdate,
)
...
from app.schemas.role import RoleResponse as CustomRoleResponse
...
async def grant_permission(
    ...
    # PermissionService.grant_permission() already writes the audit log entry
    # (action="grant_delegated_permission") — do not duplicate it here.
...
async def revoke_permission(
    ...
    # PermissionService.revoke_permission() already writes the audit log entry
    # (action="revoke_delegated_permission") — do not duplicate it here.
...
async def revoke_all_user_permissions(
    ...
    """
    # PermissionService.revoke_all_user_permissions() already writes the audit
    # log entry (action="revoke_all_delegated_permissions") — do not duplicate.
    permission_service = PermissionService(db)
...
async def list_roles(
    ...
    """
    List all roles visible to the current tenant (system roles plus its own
    custom roles).
    ...
    roles = RoleService(db).list_roles(current_user.tenant_id)
    roles_data = [
        RoleWithPermissionsResponse(
...
@router.get(
    "/custom-roles",
    response_model=StandardListResponse[CustomRoleResponse],
    status_code=status.HTTP_200_OK,
    summary="List custom roles for the current tenant",
    description="List all tenant-specific custom roles. Requires auth.manage_roles permission.",
)
async def list_custom_roles(
    current_user: Annotated[User, Depends(require_permission("auth.manage_roles"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardListResponse[CustomRoleResponse]:
    """List custom roles for the current user's tenant."""
    from app.services.role_service import RoleService

    roles = RoleService(db).list_roles(current_user.tenant_id)
    custom_roles = [r for r in roles if not r.is_system]
    return StandardListResponse(
        data=[CustomRoleResponse.model_validate(r) for r in custom_roles],
        meta=PaginationMeta(
            total=len(custom_roles),
            page=1,
            page_size=max(len(custom_roles), 1),
            total_pages=1,
        ),
...
@router.post(
    "/custom-roles",
    response_model=StandardResponse[CustomRoleResponse],
    status_code=status.HTTP_201_CREATED,
    summary="Create a custom role",
    description="Create a new tenant-specific custom role. Requires auth.manage_roles permission.",
)
async def create_custom_role_endpoint(
    ...
    current_user: Annotated[User, Depends(require_permission("auth.manage_roles"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardResponse[CustomRoleResponse]:
    """Create a custom role for the current tenant."""
    from app.services.role_service import RoleService

    role = RoleService(db).create_custom_role(
        tenant_id=current_user.tenant_id,
    ...
    create_audit_log_entry(
        ...
        resource_type="role",
        resource_id=role.id,  # type: ignore[arg-type]
    ...
    return StandardResponse(
        data=CustomRoleResponse.model_validate(role),
...
@router.get(
    ...
    description="Get a single custom role by ID. Requires auth.manage_roles permission.",
)
async def get_custom_role_endpoint(
    role_id: UUID,
    current_user: Annotated[User, Depends(require_permission("auth.manage_roles"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardResponse[CustomRoleResponse]:
    """Get a custom role by ID, scoped to the current tenant."""
    from app.services.role_service import RoleService

    role = RoleService(db).get_role_by_id(role_id, current_user.tenant_id)
    if not role:
        raise_not_found("Role", str(role_id))
    return StandardResponse(data=CustomRoleResponse.model_validate(role))
...
@router.patch(
    ...
    description="Update a tenant-specific custom role. Requires auth.manage_roles permission.",
)
async def update_custom_role_endpoint(
    role_id: UUID,
    payload: RoleUpdate,
    request: Request,
    current_user: Annotated[User, Depends(require_permission("auth.manage_roles"))],
    ...
    """Update a custom role, scoped to the current tenant."""
    from app.services.role_service import RoleService

    try:
        role = RoleService(db).update_custom_role(
            role_id, current_user.tenant_id, payload
        )
    except ValueError as e:
        raise_bad_request(code="CANNOT_MODIFY_SYSTEM_ROLE", message=str(e))
    if not role:
        raise_not_found("Role", str(role_id))
...
@router.delete(
    ...
    description="Delete a tenant-specific custom role. Requires auth.manage_roles permission.",
)
async def delete_custom_role_endpoint(
    role_id: UUID,
    request: Request,
    current_user: Annotated[User, Depends(require_permission("auth.manage_roles"))],
    db: Annotated[Session, Depends(get_db)],
    ...
    role = role_service.get_role_by_id(role_id, current_user.tenant_id)
    if not role:
        raise_not_found("Role", str(role_id))
    if role.is_system:
        raise_bad_request(
            code="CANNOT_DELETE_SYSTEM_ROLE", message="Cannot delete a system role"
        )
    role_name = role.name
    role_service.delete_custom_role(role_id)
    ...
    return StandardResponse(
        data={"message": "Role deleted"}, message="Custom role deleted successfully"
    )
...
async def assign_role(
    ...
    # Validate role exists (system or tenant-custom) -- previously unvalidated,
    # allowing an arbitrary role string to be assigned that remove_role's own
    # validation (below) would then refuse to ever remove.
    role_exists = RoleService(db).get_role_by_name(
        role_data.role, current_user.tenant_id
    )
    if not role_exists:
        raise_bad_request(
            code="INVALID_ROLE",
            message=f"Invalid role '{role_data.role}'",
        )
...
async def remove_role(
    ...
    # A tenant must always retain at least one owner, or no one could manage
    # it afterward -- block removing the "owner" role from the last user who
    # has it in this tenant.
    if role == "owner":
        owner_count = (
            db.query(UserRole)
            .join(User, User.id == UserRole.user_id)
            .filter(UserRole.role == "owner", User.tenant_id == current_user.tenant_id)
            .count()
        )
        if owner_count <= 1:
            raise_bad_request(
                code="LAST_OWNER_ROLE",
                message="Cannot remove the owner role from the last owner of this tenant",
            )
...
@router.get(
    ...
    status_code=status.HTTP_200_OK,
    summary="List users with a given role",
    description="List all users in the current tenant assigned a given global role. Requires auth.manage_users permission.",
)
async def list_users_with_role(
    ...
    """List users in the current tenant who have the given global role assigned."""
    from app.core.users.models import UserRole
    ...
    user_ids = [
        ...
        .join(User, User.id == UserRole.user_id)
        .filter(UserRole.role == role, User.tenant_id == current_user.tenant_id)
        .all()
    ]
    ...
    return StandardListResponse(
        data=data,
        meta=PaginationMeta(
            total=len(data), page=1, page_size=max(len(data), 1), total_pages=1
        ),
...
@router.get(
    ...
    status_code=status.HTTP_200_OK,
    summary="List available module-internal roles",
    description="Returns a mapping of module name to its available internal role names. Requires authentication.",
)
async def get_module_roles_catalog(
    current_user: Annotated[User, Depends(get_current_user)],
) -> StandardResponse[dict[str, list[str]]]:
    """List all module-internal role names, grouped by module."""
    from app.core.auth.permissions import MODULE_ROLES
    from app.core.module_registry import get_module_registry

    try:
        registry = get_module_registry()
        enabled_modules = set(registry.get_enabled_modules(current_user.tenant_id))
        catalog = {
            module: sorted(roles.keys())
            for module, roles in MODULE_ROLES.items()
            if module in enabled_modules
        }
    except RuntimeError:
        catalog = {
            module: sorted(roles.keys()) for module, roles in MODULE_ROLES.items()
        }
...
@router.get(
    ...
    description="Export audit logs matching the given filters as a CSV file. Requires auth.view_audit permission.",
)
async def export_audit_logs(
    ...
    page_size: int = Query(default=1000, ge=1, le=5000, description="Max rows"),
) -> Response:
    """
    Export audit logs matching the given filters as a CSV file.

    Requires: auth.view_audit

    Reuses the same filters as GET /audit-logs and the import_export module's
    CSVExporter, rather than a bespoke export implementation.
    """
    from app.core.import_export.exporters.csv_exporter import CSVExporter

    audit_service = AuditService(db)
    logs, _total = audit_service.get_audit_logs(
        tenant_id=current_user.tenant_id,
        user_id=user_id,
        action=action,
        resource_type=resource_type,
    ...
    csv_bytes = CSVExporter().export(rows, fieldnames=fieldnames)
    filename = f"audit-logs-{datetime.now(UTC).date().isoformat()}.csv"
    return Response(
        content=csv_bytes,
        media_type="text/csv",
...
@router.get(
    ...
    response_model=StandardResponse[AuditRetentionConfigResponse],
    status_code=status.HTTP_200_OK,
    summary="Get audit log retention setting",
    description="Get how many days audit logs are kept for this tenant before being eligible for purge. Requires auth.view_audit permission.",
)
async def get_audit_retention_config(
    current_user: Annotated[User, Depends(require_permission("auth.view_audit"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardResponse[AuditRetentionConfigResponse]:
    """Get the tenant's audit log retention period (defaults to 90 days)."""
    audit_service = AuditService(db)
    retention_days = audit_service.get_retention_days(current_user.tenant_id)

    return StandardResponse(
        data=AuditRetentionConfigResponse(retention_days=retention_days),
        message="Audit retention setting retrieved successfully",
    )
...
@router.put(
    ...
    response_model=StandardResponse[AuditRetentionConfigResponse],
    status_code=status.HTTP_200_OK,
    summary="Update audit log retention setting",
    description="Set how many days audit logs are kept for this tenant before being eligible for purge. Requires auth.view_audit permission.",
)
async def update_audit_retention_config(
    retention_data: AuditRetentionConfigUpdate,
    request: Request,
    current_user: Annotated[User, Depends(require_permission("auth.view_audit"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardResponse[AuditRetentionConfigResponse]:
    """Update the tenant's audit log retention period."""
    audit_service = AuditService(db)
    retention_days = audit_service.update_retention_days(
        tenant_id=current_user.tenant_id,
    ...
    return StandardResponse(
        data=AuditRetentionConfigResponse(retention_days=retention_days),
        message="Audit retention setting updated successfully",
    )


@router.post(
    "/audit-logs/purge",
    response_model=StandardResponse[AuditPurgeResponse],
    status_code=status.HTTP_200_OK,
    summary="Purge expired audit logs",
    description="Delete audit logs older than the tenant's configured retention period. Requires auth.view_audit permission.",
)
async def purge_expired_audit_logs(
    request: Request,
    current_user: Annotated[User, Depends(require_permission("auth.view_audit"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardResponse[AuditPurgeResponse]:
    """Purge audit logs older than the tenant's retention period, on demand."""
    audit_service = AuditService(db)
    deleted_count, retention_days = audit_service.purge_expired_logs(
    ...
    return StandardResponse(
        data=AuditPurgeResponse(
            deleted_count=deleted_count, retention_days=retention_days
        ),
        message=f"Purged {deleted_count} expired audit log(s)",
    )
...
async def get_user_effective_permissions(
    ...
    if "*" in effective:
        # Wildcard means "every permission" for has_permission()'s check-time
        # logic; expand it here purely for display (e.g. the permissions
        # audit table), which compares against literal catalog strings and
        # would otherwise show 0/N granted for a user with total access.
        effective = effective - {"*"}
        for group in permission_service.get_all_permissions_grouped():
            effective |= set(group["permissions"])
...
async def bulk_grant_permissions(
    ...
    create_audit_log_entry(
        ...
        resource_type="user",
        resource_id=user_id,
        details={"result": result, "requested": len(payload.permissions)},
        ip_address=ip_address,
        user_agent=user_agent,
    )
...
async def get_role_permissions_endpoint(
    ...
    permission_service = PermissionService(db)
    base_set = permission_service.get_role_permissions(role, current_user.tenant_id)
    if "*" in base_set:
        # Wildcard means "every permission" for has_permission()'s check-time
        # logic; expand it here purely for display in the permissions-editing
        # UI, which compares against literal catalog strings and would
        # otherwise show every checkbox unchecked for a role like "owner".
        base_set = base_set - {"*"}
        for group in permission_service.get_all_permissions_grouped():
            base_set |= set(group["permissions"])
...
async def set_role_permissions_endpoint(
    ...
    create_audit_log_entry(
        ...
        resource_type="role",
        details={"role": role, "result": result},
        ip_address=ip_address,
        user_agent=user_agent,
    )
