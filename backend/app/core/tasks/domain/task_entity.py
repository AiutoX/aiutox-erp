...
from aiutox_sdk.exceptions import BusinessRuleError
...
class Task:
    ...
    def _validate_business_rules(self) -> None:
        ...
        if not self.title or len(self.title.strip()) < 3:
            raise BusinessRuleError("El título debe tener al menos 3 caracteres")

        if len(self.title) > 200:
            raise BusinessRuleError("El título no puede exceder 200 caracteres")

        if self.description and len(self.description) > 2000:
            raise BusinessRuleError("La descripción no puede exceder 2000 caracteres")
        ...
        if self.estimated_duration is not None and self.estimated_duration <= 0:
            raise BusinessRuleError("La duración estimada debe ser mayor a 0")
    ...
    def assign_to(self, user_id: UUID, assigned_by: UUID) -> None:
        ...
        if user_id == self.assigned_to_id:
            raise BusinessRuleError("La tarea ya está asignada a este usuario")
    ...
    def unassign(self, unassigned_by: UUID) -> None:
        ...
        if not self.assigned_to_id:
            raise BusinessRuleError("La tarea no está asignada")
    ...
    def is_overdue(self) -> bool:
        ...
        return self.due_date < datetime.now(UTC) and self.status != TaskStatus.DONE
    ...
    def is_due_soon(self, hours_ahead: int = 24) -> bool:
        ...
        threshold = datetime.now(UTC) + timedelta(hours=hours_ahead)
        return datetime.now(UTC) < self.due_date <= threshold
    ...
    def update_fields(
        ...
        if not self.can_be_edited_by(updated_by):
            raise BusinessRuleError("No tienes permiso para editar esta tarea")
    ...
    def set_estimated_time(self, hours: int) -> None:
        ...
        if hours <= 0:
            raise BusinessRuleError("El tiempo estimado debe ser mayor a 0")
    ...
    def mark_as_complete(self, completed_by: UUID) -> None:
        ...
        if self.status == TaskStatus.DONE:
            raise BusinessRuleError("La tarea ya está completada")
