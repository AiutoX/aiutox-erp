...
class ReportingEventPublisher:
    ...
    def publish_report_failed(
        ...
        permission_denied marks a REP-002 403 (dataset access denied) rather
        than a runtime/data-source failure — still recorded for audit
        completeness (REP-005 open question), but distinguishable by the
        consumer if ever needed.
        """
        safe_publish_event(
            event_publisher=self.event_publisher,
