...
> {
  const response =
    await apiClient.get<StandardListResponse<RoleWithPermissions>>(
      "/auth/roles"
    );
 ...
 */
export async function getUserRoles(userId: string): Promise<RoleListResponse> {
...
/**
 * List users assigned a given global role (current tenant only)
 * GET /api/v1/auth/roles/{role}/users
 *
 * Requires: auth.manage_users permission
 */
...
> {
  const response =
    await apiClient.get<StandardListResponse<CustomRole>>("/auth/custom-roles");
  return response.data;
}
...
/**
 * Get the catalog of available module-internal roles, grouped by module
 * GET /api/v1/auth/module-roles/catalog
 */
export async function getModuleRolesCatalog(): Promise<
  StandardResponse<Record<string, string[]>>
> {
  const response = await apiClient.get<StandardResponse<Record<string, string[]>>>(
    "/auth/module-roles/catalog"
  );
  return response.data;
}

/**
 * Assign a module-internal role to a user
 * POST /api/v1/auth/module-roles/{user_id}/{module}
 *
...
export async function assignModuleRole(
  ...
  const response = await apiClient.post<StandardResponse<{ message: string }>>(
    `/auth/module-roles/${userId}/${module}`,
    { role_name: roleName.replace(/^internal\./, "") }
  );
  return response.data;
}
