...
interface ThemeConfigValues extends Record<string, string | boolean> {
  // Dark mode switch
  ...
  dark_panel_dark_bg: string;
  dark_panel_dark_fg: string;
  // Logos
  logo_primary: string;
  logo_white: string;
  logo_small: string;
...
export default function ThemeConfigPage() {
  ...
  const defaultValues: ThemeConfigValues = {
    ...
    panel_dark_bg: "#1E293B",
    panel_dark_fg: "#F8FAFC",
    dark_panel_dark_bg: "#0F172A",
    dark_panel_dark_fg: "#F8FAFC",
  ...
  useEffect(() => {
    ...
    const mergedTheme: Record<string, string | boolean> = {
      dark_mode_enabled: darkModeEnabled,
      ...themeLight,
      ...(themeDark
        ? Object.fromEntries(
  ...
  }, [themeLight, themeDark, darkModeEnabled, updateTheme]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    >
      <Tabs defaultValue="colors" className="space-y-6">
        ...
        <TabsContent value="colors" className="space-y-6">
          <ConfigSection
            title={t("config.theme.sectionDarkModeToggle")}
            description={t("config.theme.sectionDarkModeToggleDesc")}
          >
            <div className="flex items-center space-x-3">
              <Switch
                id="dark_mode_enabled"
                checked={Boolean(form.values.dark_mode_enabled)}
                onCheckedChange={(checked) =>
                  form.setValue("dark_mode_enabled", checked)
                }
              />
              <Label htmlFor="dark_mode_enabled">
                {t("config.theme.darkModeEnabledLabel")}
              </Label>
            </div>
          </ConfigSection>
            ...
            <TabsContent value="day" className="space-y-6">
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  />
                  <ConfigColorInput
                    label={t("config.theme.panelDarkBg")}
                    id="panel_dark_bg"
                    value={form.values.panel_dark_bg}
                    onChange={(value) => form.setValue("panel_dark_bg", value)}
                    description={t("config.theme.panelDarkBgDesc")}
                  />
                  <ConfigColorInput
                    label={t("config.theme.panelDarkFg")}
                    id="panel_dark_fg"
                    value={form.values.panel_dark_fg}
                    onChange={(value) => form.setValue("panel_dark_fg", value)}
                  />
                </div>
              </ConfigSection>
            </TabsContent>
            ...
            <TabsContent value="night" className="space-y-6">
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <ConfigColorInput
                    label={t("config.theme.panelDarkBg")}
                    id="dark_panel_dark_bg"
                    value={form.values.dark_panel_dark_bg}
                    onChange={(value) =>
                      form.setValue("dark_panel_dark_bg", value)
                    }
                    description={t("config.theme.panelDarkBgDesc")}
                  />
                  <ConfigColorInput
                    label={t("config.theme.panelDarkFg")}
                    id="dark_panel_dark_fg"
                    value={form.values.dark_panel_dark_fg}
                    onChange={(value) =>
                      form.setValue("dark_panel_dark_fg", value)
                    }
                  />
                </div>
              </ConfigSection>
