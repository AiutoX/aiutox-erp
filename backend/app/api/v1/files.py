...
def get_file_service(
    ...
    Gated on the global `files.view` permission — used for endpoints that
    operate on the file library as a whole with no single-entity context to
    inherit access from (list without entity filter, upload).
    """
    return FileService(db, tenant_id=current_user.tenant_id)


def get_file_service_for_entity(
    db: Annotated[Session, Depends(get_db)],
    current_user: Annotated[User, Depends(get_current_user)],
) -> FileService:
    """Dependency to get FileService for single-file endpoints.

    Gated on authentication only, not the global `files.*` permission — real
    access control for a specific file happens in
    `FileService.check_permissions()` (owner, entity-based inheritance via
    `app.core.files.entity_access`, or explicit FilePermission), so a user
    with no `files.*` global permission but real access to the file's owning
    business entity (e.g. `tasks.manage.all` on the task it's attached to)
    is not blocked here before that check runs.
    """
    return FileService(db, tenant_id=current_user.tenant_id)
...
@router.post(
    ...
    summary="Upload file",
    description="Upload a new file. Requires files.edit permission.",
)
async def upload_file(
    current_user: Annotated[User, Depends(require_permission("files.edit"))],
    ...
    try:
        ...
        # Preserve the real status/code (e.g. FILE_TOO_LARGE,
        # USER_STORAGE_QUOTA_EXCEEDED, TENANT_STORAGE_QUOTA_EXCEEDED) —
...
async def get_file_info(
    ...
    service: Annotated[FileService, Depends(get_file_service_for_entity)],
) -> StandardResponse[FileResponse]:
...
@router.put(
    ...
    status_code=status.HTTP_200_OK,
    summary="Update file",
    description="Update file information. Requires files.edit permission and specific file edit permission or ownership.",
)
...
@router.post(
    ...
    description="Create a new version of a file. Requires files.edit permission and specific file edit permission or ownership.",
)
...
@router.post(
    ...
    description="Add tags to a file. Requires files.edit permission and specific file edit permission or ownership.",
)
