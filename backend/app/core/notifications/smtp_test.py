...
def check_smtp_connection(config: dict[str, Any]) -> SMTPTestResult:
    ...
    # Port 465 is implicit TLS (SMTPS): the server expects a TLS ClientHello
    # as the first bytes, not a plaintext banner followed by STARTTLS. Using
    # plaintext SMTP + starttls() against such a server hangs until timeout,
    # since it never sends the plaintext banner smtplib waits for.
    use_implicit_tls = use_tls and port == 465
    smtp_class = SMTP_SSL if use_implicit_tls else SMTP

    try:
        # Create SMTP connection
        with smtp_class(host, port, timeout=timeout) as server:
            # Enable TLS via STARTTLS if configured and not already implicit
