...
gamification_event_consumer = None
calendar_event_consumer = None
reporting_execution_consumer = None
search_index_consumer = None
...
async def lifespan(app: FastAPI):
    ...
    global async_task_service, notification_event_consumer, gamification_event_consumer, calendar_event_consumer, reporting_execution_consumer, search_index_consumer, channel_subscriber
    task_scheduler = None
    ...
    # Start GamificationEventConsumer for event-driven points/badges
    try:
        from app.core.gamification.consumer import GamificationEventConsumer

        gamification_db = SessionLocal()
        gamification_event_consumer = GamificationEventConsumer(gamification_db)
        await gamification_event_consumer.start()
        logger.info("GamificationEventConsumer started successfully")
    except Exception as e:
        logger.error(f"Failed to start GamificationEventConsumer: {e}", exc_info=True)

    # Start CalendarEventConsumer to sync task schedule changes to calendar events
    try:
        from app.core.calendar.consumer import CalendarEventConsumer

        calendar_db = SessionLocal()
        calendar_event_consumer = CalendarEventConsumer(calendar_db)
        await calendar_event_consumer.start()
        logger.info("CalendarEventConsumer started successfully")
    except Exception as e:
        logger.error(f"Failed to start CalendarEventConsumer: {e}", exc_info=True)

    # Start ReportExecutionConsumer to record report execution history (REP-005)
    try:
        from app.core.reporting.consumer import ReportExecutionConsumer

        reporting_db = SessionLocal()
        reporting_execution_consumer = ReportExecutionConsumer(reporting_db)
        await reporting_execution_consumer.start()
        logger.info("ReportExecutionConsumer started successfully")
    except Exception as e:
        logger.error(f"Failed to start ReportExecutionConsumer: {e}", exc_info=True)

    # Start SearchIndexConsumer to keep search_indices in sync with real data
    try:
        from app.core.search.consumer import SearchIndexConsumer

        search_db = SessionLocal()
        search_index_consumer = SearchIndexConsumer(search_db)
        await search_index_consumer.start()
        logger.info("SearchIndexConsumer started successfully")
    except Exception as e:
        logger.error(f"Failed to start SearchIndexConsumer: {e}", exc_info=True)
    ...
    # Register entity-based file access resolvers (see app.core.files.entity_access)
    # so files attached to a business entity inherit access from whoever can
    # already act on that entity, without needing a separate files.* permission.
    try:
        from app.core.tasks.file_access import register_tasks_file_access_resolver

        register_tasks_file_access_resolver()
        logger.info("Registered tasks file access resolver")
    except Exception as e:
        logger.error(
            f"Failed to register tasks file access resolver: {e}", exc_info=True
        )

    try:
        from app.core.comments.file_access import (
            register_comments_file_access_resolver,
        )

        register_comments_file_access_resolver()
        logger.info("Registered comments file access resolver")
    except Exception as e:
        logger.error(
            f"Failed to register comments file access resolver: {e}", exc_info=True
        )
    ...
    # Seed CRM contact-message notification templates for all active tenants
    # (idempotent) — required by send_to_contact(), missing rows silently
    # fail with reason="no_template" (found during MSG-006's manual E2E)
    try:
        from app.core.tenants.models import Tenant
        from app.modules.crm.installer import CrmModuleInstaller

        seed_db = SessionLocal()
        try:
            tenants = seed_db.query(Tenant).filter(Tenant.is_active.is_(True)).all()
            for tenant in tenants:
                result = CrmModuleInstaller.seed_notification_templates(
                    seed_db, tenant.id
                )
                logger.info(
                    f"CRM notification templates seeded: tenant={tenant.id} "
                    f"created={result['created']} skipped={result['skipped']}"
                )
        finally:
            seed_db.close()
    except Exception as e:
        logger.error(f"Failed to seed CRM notification templates: {e}", exc_info=True)
    ...
    # Start CRM contact message retry scheduler (every 30s)
    try:
        from app.modules.crm.services.message_retry_scheduler import (
            start_crm_message_retry_scheduler,
        )

        start_crm_message_retry_scheduler()
        logger.info("CRM message retry scheduler started")
    except Exception as e:
        logger.error(f"Failed to start CRM message retry scheduler: {e}", exc_info=True)

    # Start comment purge scheduler (every 24h — hard-delete soft-deleted
    # comments past each tenant's configured retention window, CMT-010)
    try:
        from app.core.comments.purge_job import start_comment_purge_scheduler

        start_comment_purge_scheduler()
        logger.info("Comment purge scheduler started")
    except Exception as e:
        logger.error(f"Failed to start comment purge scheduler: {e}", exc_info=True)

    yield

    # Shutdown
    # Stop CRM contact message retry scheduler
    try:
        from app.modules.crm.services.message_retry_scheduler import (
            stop_crm_message_retry_scheduler,
        )

        stop_crm_message_retry_scheduler()
        logger.info("CRM message retry scheduler stopped")
    except Exception as e:
        logger.error(f"Error stopping CRM message retry scheduler: {e}", exc_info=True)
    ...
    # Stop comment purge scheduler
    try:
        from app.core.comments.purge_job import stop_comment_purge_scheduler

        stop_comment_purge_scheduler()
        logger.info("Comment purge scheduler stopped")
    except Exception as e:
        logger.error(f"Error stopping comment purge scheduler: {e}", exc_info=True)
    ...
    # Stop GamificationEventConsumer
    if gamification_event_consumer:
        try:
            await gamification_event_consumer.stop()
            if hasattr(gamification_event_consumer, "db"):
                gamification_event_consumer.db.close()
            logger.info("GamificationEventConsumer stopped successfully")
        except Exception as e:
            logger.error(
                f"Error stopping GamificationEventConsumer: {e}", exc_info=True
            )

    # Stop CalendarEventConsumer
    if calendar_event_consumer:
        try:
            await calendar_event_consumer.stop()
            if hasattr(calendar_event_consumer, "db"):
                calendar_event_consumer.db.close()
            logger.info("CalendarEventConsumer stopped successfully")
        except Exception as e:
            logger.error(f"Error stopping CalendarEventConsumer: {e}", exc_info=True)

    # Stop ReportExecutionConsumer
    if reporting_execution_consumer:
        try:
            await reporting_execution_consumer.stop()
            if hasattr(reporting_execution_consumer, "db"):
                reporting_execution_consumer.db.close()
            logger.info("ReportExecutionConsumer stopped successfully")
        except Exception as e:
            logger.error(f"Error stopping ReportExecutionConsumer: {e}", exc_info=True)

    # Stop SearchIndexConsumer
    if search_index_consumer:
        try:
            await search_index_consumer.stop()
            if hasattr(search_index_consumer, "db"):
                search_index_consumer.db.close()
            logger.info("SearchIndexConsumer stopped successfully")
        except Exception as e:
            logger.error(f"Error stopping SearchIndexConsumer: {e}", exc_info=True)
