...
from aiutox_sdk.reporting import ReportingEngine
from sqlalchemy.orm import Session

from app.core.tasks.reporting.data_source import TasksDataSource
from app.core.tasks.reporting.definitions import (
...
def create_default_reports(
    ...
    """
    from aiutox_sdk.reporting import ReportingService
    ...
    for config in report_configs:
        try:
            report = service.create_report(
                ...
                visualization_type=str(config["visualization_type"]),
                ...
                filters=config["report_definition"].filters,  # type: ignore[attr-defined]
                config=config["report_definition"].config,  # type: ignore[attr-defined]
            )
            created_report_ids.append(UUID(str(report.id)))
            logger.info(
