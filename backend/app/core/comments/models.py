...
class CommentRevision(Base):
    """A snapshot of a comment's content taken immediately before an edit
    overwrites it (CMT-008). Visible only to comments.manage — not the
    comment's own author — for moderation/accountability, not as a
    user-facing "edit history" feature."""
    ...
    # The content as it was BEFORE this edit overwrote it — not the edit's
    # new content, which lives on the Comment row itself.
    content = Column(Text, nullable=False)
    ...
    comment = relationship("Comment", backref="revisions")

    __table_args__ = (
        Index("idx_comment_revisions_comment", "comment_id", "created_at"),
    )

    def __repr__(self) -> str:
        return f"<CommentRevision(id={self.id}, comment_id={self.comment_id})>"
