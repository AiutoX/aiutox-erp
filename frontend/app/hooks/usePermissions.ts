...
import { getModulePermissions, getAllPermissions } from "~/lib/api/modules.api";

/**
 * Check a required permission against a user's permission set.
 * Mirrors the backend wildcard matcher (app/core/auth/permissions.py::has_permission):
 * exact match, "module.*", "*.action", "*.*.action", and total wildcards "*"/"*.*".
 */
function matchesPermission(
  userPermissions: string[] | undefined,
  required: string
): boolean {
  // Empty/falsy `required` is the project convention for "no permission
  // needed" (e.g. WidgetManifest.permission == null -> widget.permission ??
  // "" in dashboard.tsx). Must short-circuit before the wildcard branches
  // below: "".endsWith(".view") is false, so a *.*.view-style wildcard user
  // (anyone without the literal "*"/"*.*" permission) would otherwise fail
  // this check and have the widget hidden — the opposite of "no gate needed".
  // Found live during the widgets Phase 7.5 security audit (SEC-W-005,
  // 2026-08-04): any WidgetManifest.permission == null widget was invisible
  // to every user except one holding the literal wildcard permission.
...
export function usePermissions() {
  ...
  const hasPermission = useMemo(() => {
    return (permission: string) => matchesPermission(user?.permissions, permission);
  }, [user?.permissions]);
  ...
  return useMemo(
    () => ({
      ...
      hasRole: (role: string) => {
        ...
        return user.roles.some((r) => r.role === role); // Compare role string to UserRole.role
      },
      ...
      hasAnyRole: (roles: string[]) => {
        if (!user?.roles) return false;
        return roles.some((r) => user.roles?.some((ur) => ur.role === r)); // Compare role string to UserRole.role
      },
      ...
      getModulePermissions: (moduleId: string, tenantId?: string) => {
        ...
        return user.permissions.filter(
          (p) =>
            p === `${moduleId}.*` || p.startsWith(`${moduleId}.`) || p === "*"
        );
      },
    }),
    [user, hasPermission]
      ...
      try {
        ...
        for (const perm of response.data) {
          const permissionString =
            typeof perm === "string"
              ? perm
              : perm && typeof perm === "object" && "permission" in perm
                ? String((perm as Record<string, unknown>).permission)
                : "";
