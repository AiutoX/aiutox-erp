...
from app.core.gamification.models import Badge, GamificationEvent, UserBadge
...
class BadgeService:
    ...
    def check_and_award_badges(
        ...
        for badge in active_badges:
            if self._user_has_badge(user_id, UUID(str(badge.id))):
                continue
    ...
    def update_badge(self, badge_id: UUID, badge_data: dict[str, Any]) -> Badge:
        """Update an existing badge definition (admin only).

        Args:
            badge_id: Badge to update, scoped to this service's tenant
            badge_data: Fields to update (name, description, icon, criteria,
                points_value) — only provided keys are changed

        Returns:
            Updated Badge

        Raises:
            ValueError: If no badge with this id exists for the tenant
        """
        badge = self._get_badge_or_raise(badge_id)

        for field in ("name", "description", "icon", "criteria", "points_value"):
            if field in badge_data:
                setattr(badge, field, badge_data[field])

        self.db.commit()
        self.db.refresh(badge)
        return badge

    def deactivate_badge(self, badge_id: UUID) -> Badge:
        """Deactivate a badge (soft delete, matches list_badges' active_only filter).

        Args:
            badge_id: Badge to deactivate, scoped to this service's tenant
    ...
    def _get_badge_or_raise(self, badge_id: UUID) -> Badge:
        """Fetch a badge scoped to the tenant, or raise ValueError."""
        badge = (
            self.db.query(Badge)
            .filter(Badge.tenant_id == self.tenant_id, Badge.id == badge_id)
            .first()
        )
        if badge is None:
