...
from app.core.activities.models import Activity
...
class ActivityService:
    ...
    def get_activities(
        ...
        Args:
            ...
            entity_id: Entity ID
            tenant_id: Tenant ID
            activity_type: Filter by a single activity type (optional)
            skip: Skip records
            limit: Limit records
            activity_types: Filter by multiple activity types (optional, takes
                precedence over activity_type when both are given)
            date_from: Only include activities created on/after this datetime
            date_to: Only include activities created on/before this datetime
    ...
    def count_activities(
        ...
        Args:
            ...
            tenant_id: Tenant ID
            activity_type: Filter by a single activity type (optional)
            activity_types: Filter by multiple activity types (optional)
            date_from: Only include activities created on/after this datetime
            date_to: Only include activities created on/before this datetime
