...
from app.schemas.file_config import (
    ...
    StorageConfigUpdate,
    StorageQuotaConfigResponse,
    StorageQuotaConfigUpdate,
    StorageStatsResponse,
    ThumbnailConfigResponse,
    ThumbnailConfigUpdate,
    UserQuotaOverrideRequest,
    UserQuotaUsageResponse,
)
...
def _build_module_list_item(
    ...
    """Assemble a ModuleListItem from a registry module instance."""
    raw_deps: list[str] = (
        getattr(module_instance, "get_dependencies", lambda: [])() or []
    )
    dependencies = [
        str(d) for d in (raw_deps if isinstance(raw_deps, list) else []) if d
    ]

    def _parse_nav(items: list[dict]) -> list[ModuleNavigationItemSchema]:
        try:
            return [ModuleNavigationItemSchema.model_validate(i) for i in items]
        except Exception:  # noqa: BLE001
            return []

    def _parse_schema(fields: list[Any]) -> list[ModuleSettingField]:
        try:
            return [ModuleSettingField.model_validate(f) for f in fields]
        except Exception:  # noqa: BLE001
            return []

    models: list[Any] = getattr(module_instance, "get_models", lambda: [])() or []

    return ModuleListItem(
        id=str(module_id),
        name=str(getattr(module_instance, "module_name", "") or ""),
        type=str(getattr(module_instance, "module_type", "business") or "business"),
        enabled=bool(is_enabled),
        dependencies=dependencies,
        description=str(getattr(module_instance, "description", "") or ""),
        has_router=getattr(module_instance, "get_router", lambda: None)() is not None,
        model_count=len(models) if isinstance(models, list) else 0,
        navigation_items=_parse_nav(
            getattr(module_instance, "get_navigation_items", lambda: [])() or []
        ),
        settings_links=_parse_nav(
            getattr(module_instance, "get_settings_navigation", lambda: [])() or []
        ),
        settings_schema=_parse_schema(
            getattr(module_instance, "get_settings_schema", lambda: [])() or []
        ),
    )
...
@router.get(
    ...
    description=(
        "List all available modules with their enabled status. Any authenticated "
        "user can call this endpoint — it only drives navigation/sidebar rendering "
        "and exposes no data beyond what every module already declares publicly "
        "(nav labels, icons, settings schema). Per-module data and actions remain "
        "gated by each module's own permissions."
    ),
...
async def list_modules(
    ...
    """List all available modules with their enabled status. Any authenticated user."""
    import logging
    ...
    try:
        ...
        return StandardListResponse(
            data=[],
            meta=PaginationMeta(total=0, page=1, page_size=1, total_pages=0),
        )

    modules_list: list[ModuleListItem] = []
    for module_id, module_instance in registry.get_all_modules().items():
        try:
            ...
            logger.warning(
                "Skipping module %s: failed to build list item",
                module_id,
                exc_info=True,
            )
            continue
...
@router.get(
    ...
    description=(
        "Get detailed information about a module. Any authenticated user can call "
        "this — same navigation/settings-schema data as GET /config/modules, just "
        "scoped to one module. Per-module data and actions remain gated by each "
...
async def get_module_info(
    ...
    """
    Get detailed information about a module. Any authenticated user.
    ...
    Raises:
        APIException: If module not found.
    """
    try:
        registry = get_module_registry()
    except RuntimeError:
    ...
    raw_schema = (
        module.get_settings_schema() if hasattr(module, "get_settings_schema") else []
    )
    try:
        parsed_schema = [ModuleSettingField.model_validate(f) for f in raw_schema]
    except Exception:
        parsed_schema = []
...
async def enable_module(
    ...
    try:
        ...
        # Registry not initialized — initialize it on-the-fly
        try:
            from pathlib import Path as _Path

            from app.core.module_registry import (
                ModuleRegistry as _ModuleRegistry,  # noqa: N814
            )
            from app.core.module_registry import set_module_registry as _set_reg

            _reg = _ModuleRegistry()
            _reg.discover_modules(_Path(__file__).parent.parent.parent)
            _reg.resolve_dependencies()
            _set_reg(_reg)
            registry = _reg
        except Exception:
            raise_not_found("Module", module_id)
...
async def disable_module(
    ...
    if module_type == "core" and module_id in ["auth", "users"]:
        raise_bad_request(
...
async def update_theme_property(
    ...
    if key in color_keys or (
        ...
        try:
            validate_color(key, str(config_update.value))
...
@router.get(
    "/files/quota",
    response_model=StandardResponse[StorageQuotaConfigResponse],
    status_code=status.HTTP_200_OK,
    summary="Get storage quota configuration",
    description=(
        "Get tenant-wide and default personal storage quota configuration, "
        "plus current tenant-wide usage. Requires system.configure permission."
    ),
)
async def get_storage_quota_config(
    current_user: Annotated[User, Depends(require_permission("system.configure"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardResponse[StorageQuotaConfigResponse]:
    """Get storage quota configuration + current tenant usage."""
    ...
    return StandardResponse(
        data=StorageQuotaConfigResponse(
            tenant_max_bytes=service.get_tenant_quota_bytes(tenant_id),
            tenant_warning_thresholds=service.get_tenant_thresholds(tenant_id),
            tenant_usage_bytes=service.get_tenant_usage_bytes(tenant_id),
            user_default_max_bytes=service.get_default_user_quota_bytes(tenant_id),
            user_warning_thresholds=service.config_service.get(
                tenant_id, "files", "quota.user_warning_thresholds", [80, 100]
            ),
        ),
        message="Storage quota configuration retrieved successfully",
    )
...
@router.put(
    ...
    response_model=StandardResponse[StorageQuotaConfigResponse],
    status_code=status.HTTP_200_OK,
    summary="Update storage quota configuration",
    description=(
        "Update tenant-wide storage quota, default personal quota, and their "
        "warning thresholds. Requires system.configure permission."
    ),
)
async def update_storage_quota_config(
    current_user: Annotated[User, Depends(require_permission("system.configure"))],
    db: Annotated[Session, Depends(get_db)],
    quota_data: StorageQuotaConfigUpdate,
) -> StandardResponse[StorageQuotaConfigResponse]:
    ...
    if quota_data.tenant_max_bytes is not None:
        service.update_tenant_quota_bytes(
            tenant_id, quota_data.tenant_max_bytes, user_id=current_user.id
        )
    if quota_data.tenant_warning_thresholds is not None:
        service.update_tenant_thresholds(
            tenant_id, quota_data.tenant_warning_thresholds, user_id=current_user.id
        )
    if quota_data.user_default_max_bytes is not None:
        service.update_default_user_quota_bytes(
            tenant_id, quota_data.user_default_max_bytes, user_id=current_user.id
        )
    if quota_data.user_warning_thresholds is not None:
        service.update_default_user_thresholds(
            tenant_id, quota_data.user_warning_thresholds, user_id=current_user.id
        )
    ...
    return StandardResponse(
        ...
        message="Storage quota configuration updated successfully",
    )


@router.get(
    "/files/quota/users",
    response_model=StandardListResponse[UserQuotaUsageResponse],
    status_code=status.HTTP_200_OK,
    summary="Get per-user storage quota usage",
    description=(
        "Get every user's personal storage quota and current usage, for the "
        "tenant admin's dashboard. Requires system.configure permission."
    ),
)
async def get_user_quota_usage(
    current_user: Annotated[User, Depends(require_permission("system.configure"))],
    db: Annotated[Session, Depends(get_db)],
) -> StandardListResponse[UserQuotaUsageResponse]:
    """Get per-user storage quota usage breakdown."""
    from app.core.files.quota_service import QuotaService
    from app.core.users.models import User as UserModel
    ...
    breakdown = {
        row["user_id"]: row["bytes_used"]
        for row in service.get_tenant_usage_breakdown(tenant_id)
    }
    users = db.query(UserModel).filter(UserModel.tenant_id == tenant_id).all()
    preference_repo = PreferenceRepository(db)

    results = []
    for user in users:
        override = preference_repo.get_user_preference(
            user.id, tenant_id, "storage_quota", "max_bytes"
        )
        results.append(
            UserQuotaUsageResponse(
                user_id=str(user.id),
                ...
                quota_max_bytes=service.get_user_quota_bytes(user.id, tenant_id),
                usage_bytes=breakdown.get(user.id, 0),
    ...
    return StandardListResponse(
        ...
        meta={
            "total": len(results),
            "page": 1,
            "page_size": len(results) if results else 20,
            "total_pages": 1,
        },
        message="Per-user storage quota usage retrieved successfully",
    )


@router.put(
    "/files/quota/users/{target_user_id}",
    response_model=StandardResponse[UserQuotaUsageResponse],
    status_code=status.HTTP_200_OK,
    summary="Set a per-user storage quota override",
    description=(
        "Set a personal storage quota override for one user, distinct from "
        "the tenant default. Requires system.configure permission."
    ),
)
async def update_user_quota_override(
    target_user_id: Annotated[UUID, Path(..., description="Target user ID")],
    current_user: Annotated[User, Depends(require_permission("system.configure"))],
    db: Annotated[Session, Depends(get_db)],
    override_data: UserQuotaOverrideRequest,
) -> StandardResponse[UserQuotaUsageResponse]:
    """Set a per-user storage quota override."""
    from app.core.files.quota_service import QuotaService
    from app.core.users.models import User as UserModel
    ...
    target_user = (
        db.query(UserModel)
        .filter(UserModel.id == target_user_id, UserModel.tenant_id == tenant_id)
        .first()
    )
    if not target_user:
        raise APIException(
            ...
            code="USER_NOT_FOUND",
            message=f"User {target_user_id} not found in this tenant.",
        )

    service = QuotaService(db)
    service.set_user_quota_override(target_user_id, tenant_id, override_data.max_bytes)

    return StandardResponse(
        data=UserQuotaUsageResponse(
            user_id=str(target_user.id),
            email=target_user.email,
            full_name=target_user.full_name,
            quota_max_bytes=service.get_user_quota_bytes(target_user_id, tenant_id),
            usage_bytes=service.get_user_usage_bytes(target_user_id, tenant_id),
            has_override=True,
        ),
        message="Personal storage quota override updated successfully",
    )


@router.get(
    "/files/quota/me",
    response_model=StandardResponse[MyQuotaResponse],
    status_code=status.HTTP_200_OK,
    summary="Get my own storage quota and usage",
    description=(
        "Get the current user's own effective personal storage quota, "
        "usage, and warning thresholds. Any authenticated user may call "
        "this for their own data — no system.configure permission required."
    ),
)
async def get_my_storage_quota(
    current_user: Annotated[User, Depends(get_current_user)],
    db: Annotated[Session, Depends(get_db)],
) -> StandardResponse[MyQuotaResponse]:
    """Get the current user's own storage quota and usage."""
    from app.core.files.quota_service import QuotaService
    ...
    return StandardResponse(
        data=MyQuotaResponse(
            quota_max_bytes=service.get_user_quota_bytes(current_user.id, tenant_id),
            usage_bytes=service.get_user_usage_bytes(current_user.id, tenant_id),
            warning_thresholds=service.get_user_thresholds(current_user.id, tenant_id),
        ),
        message="Personal storage quota retrieved successfully",
    )
...
@router.post(
    "/notifications/channels/smtp/test-email",
    response_model=StandardResponse[dict],
    status_code=status.HTTP_200_OK,
    summary="Send a test email via the configured SMTP channel",
    description=(
        "Send a real test email to a chosen recipient using the tenant's saved "
        "SMTP configuration. Requires notifications.manage permission."
    ),
    responses={
        403: {
            "description": "Insufficient permissions",
...
async def send_smtp_test_email(
    ...
    """
    Send a real test email through the tenant's configured SMTP channel.

    Requires: notifications.manage

    Args:
        payload: Recipient email address for the test message.
        request: FastAPI request object (required by the rate limiter).
        current_user: Current authenticated user (must have notifications.manage).
        db: Database session.

    Returns:
        StandardResponse confirming the test email was sent.
    """
    ...
    try:
        await notification_service.send_test_email(
            tenant_id=current_user.tenant_id,
            recipient_email=payload.email,
        )
    except ValueError as e:
        raise_bad_request("SMTP_NOT_ENABLED", str(e))
    except Exception as e:
        raise_bad_request(
    ...
    return StandardResponse(
        data={"success": True, "recipient": payload.email},
