 ...
 * POST /api/v1/calendar/calendars/{id}/shares
 *
 * Requires: calendar.manage permission
 */
...
): Promise<StandardResponse<CalendarShare>> {
  const response = await apiClient.post<StandardResponse<CalendarShare>>(
    `/calendar/calendars/${calendarId}/shares`,
    payload
  );
  return response.data;
}
...
/**
 * List a calendar's shares
 * GET /api/v1/calendar/calendars/{id}/shares
 *
 * Requires: calendar.manage permission
 */
export async function listCalendarShares(
  calendarId: string
): Promise<StandardListResponse<CalendarShare>> {
  const response = await apiClient.get<StandardListResponse<CalendarShare>>(
    `/calendar/calendars/${calendarId}/shares`
  );
  return response.data;
}
 ...
 * DELETE /api/v1/calendar/calendars/{id}/shares/{userId}
 *
 * Requires: calendar.manage permission
 */
export async function revokeCalendarShare(
  ...
  const response = await apiClient.delete<StandardResponse<null>>(
    `/calendar/calendars/${calendarId}/shares/${userId}`
  );
  return response.data;
}
 ...
 * POST /api/v1/calendar/events/{id}/send-ics
 *
 * Requires: calendar.events.export permission
 */
export async function sendEventIcs(
  eventId: string,
  payload: { recipient_email: string; message?: string }
): Promise<StandardResponse<null>> {
  const response = await apiClient.post<StandardResponse<null>>(
    `/calendar/events/${eventId}/send-ics`,
    payload
  );
  return response.data;
}
