...
// Fallback while useConditionOperators() is loading — kept in sync with the
// backend's canonical vocabulary in condition_evaluator.py.
const FALLBACK_OPERATOR_LABELS: [string, string][] = [
  ["==", "= (equals)"],
  ["!=", "≠ (not equals)"],
  [">", "> (greater)"],
  [">=", "≥ (greater or equal)"],
...
function ConditionConfig({
  ...
  const operatorOptions: [string, string][] = operatorsResponse?.data?.length
    ? operatorsResponse.data.map((op) => [op.operator, op.description])
    : FALLBACK_OPERATOR_LABELS;
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-2">
        <Checkbox
          id="condition-config-negate"
          checked={Boolean(data.negate)}
          onCheckedChange={(checked) => onChange({ negate: checked === true })}
        />
        <Label
          htmlFor="condition-config-negate"
