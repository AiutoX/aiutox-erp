...
from aiutox_sdk.tasks import TaskService, get_task_file_service
...
def _validate_file_url(url: str) -> None:
    ...
    Relative paths (e.g. "/files/{path}") are the expected shape for the
    local storage backend (LocalStorageBackend.get_url) and never leave this
    server, so they are exempt from the SSRF/scheme checks below, which only
    apply to absolute URLs (S3 and other external backends).
    """
    if url.startswith("/"):
        return
