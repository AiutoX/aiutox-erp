...
export function TaskEdit({
  ...
  const { data: taskSettings } = useTaskModuleSettings();
  ...
  // start_at/end_at are always real scheduled instants (never all-day, this
  // form's "all_day" toggle only governs those two fields, see below).
  const toDateTimeLocalValue = (value?: string | null) =>
    dueDateToInputValue(value, false);
  const toUTCISOString = (value?: string | null) =>
    dueDateInputValueToIso(value ?? "", false);
  ...
  useEffect(() => {
    if (task) {
      setFormData({
        ...
        due_date: dueDateToInputValue(task.due_date, Boolean(task.all_day)),
      ...
      <DialogContent className="sm:max-w-140 max-h-[85vh] overflow-y-auto">
        ...
        >
          ...
          {task && (task.start_at || task.end_at) ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label htmlFor="edit-due-date-all-day">
                  {t("calendar.events.allDay")}
                </Label>
                <Switch
                  id="edit-due-date-all-day"
                  checked={Boolean(formData.all_day)}
                  onCheckedChange={(checked) => {
                    // Re-derive the input value in the new format from the
                    // underlying instant, instead of leaving a stale
                    // datetime-local/date-only string in the other shape.
          ...
          {task &&
            (formData.start_at || formData.due_date || formData.end_at) && (
          ...
          {task && (
            <div className="space-y-4 pt-4 border-t">
              {(() => {
                console.warn(
                  "[TaskEdit] Rendering FileUploader with task.id:",
                  task?.id
                );
                return null;
              })()}
          ...
          {task && (
            <div className="pt-4 border-t">
              <EntityCommentList
                ...
                showTitle
                attachmentsEnabled={
                  taskSettings?.data?.comments_attachments_enabled ?? false
                }
              />
            </div>
          )}
              ...
              <>
                ...
                >
                  {deleteTask.isPending
                    ...
                    : t("common.confirmDelete") || "Confirmar Eliminación"}
                </Button>
              </>
            )}
