...
from app.core.files.models import Folder, FolderPermission
...
class FolderService:
    ...
    async def update_folder(
        ...
        if name and name != folder.name:
            existing = self.repository.get_by_parent(
                UUID(str(folder.parent_id)) if folder.parent_id else None, tenant_id
            )
            if any(
        ...
        update_data_dict: dict[str, Any] = update_data  # type: ignore[assignment]
        if color is not None:
            update_data_dict["color"] = color
        if icon is not None:
            update_data_dict["icon"] = icon
        if metadata is not None:
            update_data_dict["metadata"] = metadata

        updated_folder = self.repository.update(folder_id, tenant_id, update_data_dict)
    ...
    async def delete_folder(
        ...
        try:
            ...
            if deleted:
                # Publish event (non-critical — swallow Redis errors)
                try:
                    await self.event_publisher.publish(
                    ...
                    logger.warning(
                        f"Failed to publish folder.deleted event (non-critical): {pe}"
                    )
            return deleted
    ...
    def set_folder_permissions(
        ...
        """
        from app.core.users.models import Organization, User
    ...
    def check_inherited_permissions(
        ...
        while current:
            permissions = self.repository.get_permissions(
                UUID(str(current.id)), tenant_id
            )
            for perm in permissions:
