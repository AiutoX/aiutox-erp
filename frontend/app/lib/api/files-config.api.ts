...
export interface StorageQuotaConfig {
  tenant_max_bytes: number;
  tenant_warning_thresholds: number[];
  tenant_usage_bytes: number;
  user_default_max_bytes: number;
  user_warning_thresholds: number[];
}

export interface StorageQuotaConfigUpdate {
  tenant_max_bytes?: number;
  tenant_warning_thresholds?: number[];
  user_default_max_bytes?: number;
  user_warning_thresholds?: number[];
...
export async function getStorageConfig(): Promise<
  StandardResponse<StorageConfig>
> {
...
export async function testS3Connection(
  ...
  >("/config/files/storage/test", config);
  return response.data;
}
 ...
 * GET /api/v1/config/files/quota
 *
 * Requires: system.configure permission
 */
export async function getStorageQuotaConfig(): Promise<
  StandardResponse<StorageQuotaConfig>
> {
  const response = await apiClient.get<StandardResponse<StorageQuotaConfig>>(
    "/config/files/quota"
  );
  return response.data;
}
 ...
 * PUT /api/v1/config/files/quota
 *
 * Requires: system.configure permission
 */
export async function updateStorageQuotaConfig(
  config: StorageQuotaConfigUpdate
): Promise<StandardResponse<StorageQuotaConfig>> {
  const response = await apiClient.put<StandardResponse<StorageQuotaConfig>>(
    "/config/files/quota",
    config
  );
  return response.data;
 ...
 * GET /api/v1/config/files/quota/users
 *
 * Requires: system.configure permission
 */
export async function getUserQuotaUsage(): Promise<{
  ...
  const response = await apiClient.get<{
    data: UserQuotaUsage[];
    meta: { total: number; page: number; page_size: number; total_pages: number };
  }>("/config/files/quota/users");
  return response.data;
}

/**
 * Set a per-user storage quota override
 * PUT /api/v1/config/files/quota/users/{userId}
 *
 * Requires: system.configure permission
 */
export async function updateUserQuotaOverride(
  userId: string,
  maxBytes: number
): Promise<StandardResponse<UserQuotaUsage>> {
  const response = await apiClient.put<StandardResponse<UserQuotaUsage>>(
    `/config/files/quota/users/${userId}`,
    { max_bytes: maxBytes }
  );
  return response.data;
}
...
/**
 * Get my own storage quota and usage
 * GET /api/v1/config/files/quota/me
 */
export async function getMyStorageQuota(): Promise<
  StandardResponse<MyQuota>
> {
  const response = await apiClient.get<StandardResponse<MyQuota>>(
    "/config/files/quota/me"
  );
  return response.data;
}
 ...
 * Requires: system.configure permission
 */
export async function getThumbnailConfig(): Promise<
  StandardResponse<ThumbnailConfig>
> {
