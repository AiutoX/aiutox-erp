...
class Calendar(Base):
    ...
    shares = relationship(
        "CalendarShare", back_populates="calendar", cascade="all, delete-orphan"
    )
...
class CalendarShare(Base):
    """Grants a user access to a calendar owned by someone else."""
    ...
    permission_level = Column(
        String(20), nullable=False, default=CalendarSharePermission.VIEW
    )
    shared_by = Column(
        PG_UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="SET NULL"),
    ...
    __table_args__ = (
        Index("idx_calendar_shares_calendar", "tenant_id", "calendar_id"),
        Index("idx_calendar_shares_user", "tenant_id", "user_id"),
        UniqueConstraint("calendar_id", "user_id", name="uq_calendar_share"),
    )

    def __repr__(self) -> str:
        return (
            f"<CalendarShare(id={self.id}, calendar_id={self.calendar_id}, "
            f"user_id={self.user_id}, level={self.permission_level})>"
        )
