...
export default function TaskSettingsPage() {
  ...
  const groupedSettings = useMemo(
    () => [
      ...
      {
        key: "comments_attachments_enabled",
        label: t("tasks.settings.commentsAttachments"),
        description: t("tasks.settings.commentsAttachmentsDescription"),
      },
    ],
    [t]
  );
  ...
  const handleToggle = (
    key: keyof TaskModuleSettingsUpdate,
    value: boolean
  ) => {
    ...
    >
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {t("tasks.settings.quickActions") || "Acciones Rápidas"}
            </CardTitle>
          </CardHeader>
          <CardContent>
          ...
          <CardContent className="space-y-6">
            {groupedSettings.map((item) => (
              ...
              >
                ...
                <Switch
                  checked={Boolean(
                    settings?.[item.key as keyof TaskModuleSettingsUpdate]
                  )}
