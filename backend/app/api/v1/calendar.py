...
@router.post(
    ...
    response_model=StandardResponse[CalendarShareResponse],
    status_code=http_status.HTTP_201_CREATED,
    summary="Share calendar",
    description="Share a calendar with another user. Requires calendar.manage permission.",
)
async def share_calendar(
    ...
    current_user: Annotated[User, Depends(require_permission("calendar.manage"))],
    service: Annotated[CalendarService, Depends(get_calendar_service)],
) -> StandardResponse[CalendarShareResponse]:
    ...
    return StandardResponse(
        data=CalendarShareResponse.model_validate(share),
...
@router.get(
    "/calendars/{calendar_id}/shares",
    response_model=StandardListResponse[CalendarShareResponse],
    status_code=http_status.HTTP_200_OK,
    summary="List calendar shares",
    description="List users a calendar is shared with. Requires calendar.manage permission.",
)
async def list_calendar_shares(
    calendar_id: Annotated[UUID, Path(..., description="Calendar ID")],
    current_user: Annotated[User, Depends(require_permission("calendar.manage"))],
    service: Annotated[CalendarService, Depends(get_calendar_service)],
) -> StandardListResponse[CalendarShareResponse]:
    """List shares for a calendar."""
    shares = service.get_calendar_shares(calendar_id, current_user.tenant_id)

    return StandardListResponse(
        data=[CalendarShareResponse.model_validate(s) for s in shares],
        meta={
            "total": len(shares),
            "page": 1,
            "page_size": max(len(shares), 1),
            "total_pages": 1,
        },
    )
...
@router.delete(
    "/calendars/{calendar_id}/shares/{user_id}",
    status_code=http_status.HTTP_204_NO_CONTENT,
    summary="Revoke calendar share",
    description="Revoke a user's access to a shared calendar. Requires calendar.manage permission.",
)
async def revoke_calendar_share(
    ...
    success = service.revoke_calendar_share(
        calendar_id, user_id, current_user.tenant_id
    )
    if not success:
        raise APIException(
            status_code=http_status.HTTP_404_NOT_FOUND,
            code="CALENDAR_SHARE_NOT_FOUND",
            message="Calendar share not found",
        )
...
async def list_events(
    ...
    calendar_id: str | None = Query(
        None, description="Filter by calendar ID (comma-separated for multiple)"
    ),
    ...
    if calendar_id:
        try:
            calendar_ids = [UUID(cid) for cid in calendar_id.split(",") if cid]
        except ValueError as exc:
            raise APIException(
                status_code=http_status.HTTP_422_UNPROCESSABLE_ENTITY,
                code="VALIDATION_ERROR",
                message="calendar_id must be a UUID or comma-separated list of UUIDs",
            ) from exc
...
@router.post(
    "/events/{event_id}/send-ics",
    response_model=StandardResponse[None],
    status_code=http_status.HTTP_200_OK,
    summary="Send event as .ics",
    description=(
        "Generate a .ics file for a single event and email it to an external "
        "address. Requires calendar.events.export permission."
    ),
)
async def send_event_ics(
    event_id: Annotated[UUID, Path(..., description="Event ID")],
    current_user: Annotated[
        User, Depends(require_permission("calendar.events.export"))
    ],
    service: Annotated[CalendarService, Depends(get_calendar_service)],
    send_data: EventIcsSendRequest,
) -> StandardResponse[None]:
    """Send a single event as a .ics attachment to an external email address."""
    try:
        sent = await service.send_event_ics(
            event_id=event_id,
            tenant_id=current_user.tenant_id,
    ...
    return StandardResponse(data=None, message="Event .ics sent successfully")
...
async def cancel_event(
    ...
    event = service.cancel_event(event_id, current_user.tenant_id, current_user.id)
    if not event:
        raise APIException(
            status_code=http_status.HTTP_404_NOT_FOUND,
            code="EVENT_NOT_FOUND",
...
async def list_event_resources(
    ...
    return StandardListResponse(
        ...
        meta={
            ...
            "page": 1,
            "page_size": max(len(event_resources), 1),  # Minimum page_size is 1
            "total_pages": 1,
        },
    )
