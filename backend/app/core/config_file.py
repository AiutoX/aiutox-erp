...
class Settings(BaseSettings):
    ...
    # Security
    SECRET_KEY: str = "change-me-in-production-replace-this"
    ALGORITHM: str = "HS256"
    ...
    # Default tenant bootstrap (used by DefaultTenantSeeder)
    ...
    EVOLUTION_RATE_LIMIT_PER_MINUTE: int = 60  # Max messages per tenant per minute
    ...
    # Plugin Architecture — Event Schema Validation
    # "strict"  → raise SchemaValidationError if payload violates schema (dev/test default)
    # "lenient" → log warning and continue (prod default)
    AIUTOX_EVENT_SCHEMA_MODE: Literal["strict", "lenient"] = "lenient"

    model_config = SettingsConfigDict(
        env_file=[
            ".env.docker",
            ".env",
        ],  # .env takes priority over .env.docker locally
