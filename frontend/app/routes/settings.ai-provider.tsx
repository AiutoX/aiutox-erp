...
const DEFAULT_FORM: FormState = {
  ...
  api_key: "",
  base_url: "",
  model_conversation: PROVIDER_DEFAULTS.anthropic.conversation,
  model_classifier: PROVIDER_DEFAULTS.anthropic.classifier,
  model_embeddings: PROVIDER_DEFAULTS.anthropic.embeddings,
};
...
export default function AIProviderSettingsPage() {
  ...
  const handleProviderChange = (value: string) => {
    ...
    setForm({
      ...
      api_key: "",
      base_url: "",
      model_conversation: providerDefaults.conversation,
      model_classifier: providerDefaults.classifier,
      model_embeddings: providerDefaults.embeddings,
    });
    setInvalidKeyError(false);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setInvalidKeyError(false);
    // Model fields are guaranteed non-empty here — the submit button is disabled
    // otherwise — since the backend's ProviderConfigCreate requires them as
    // non-null strings (see EH-018).
    saveConfig({
      provider_type: form.provider_type,
      // Send null, not "", when the key is unchanged: the backend's
      // ProviderConfigCreate accepts `str | None` but applies min_length=10 to
      // any string it receives, so "" fails validation and the whole save 422s
      // — blocking every edit that doesn't also replace the key (UX-010).
      api_key: form.api_key || null,
      base_url: form.base_url || null,
      model_conversation: form.model_conversation,
      model_classifier: form.model_classifier,
      model_embeddings: form.model_embeddings,
    });
  };
      ...
      <div className="max-w-2xl space-y-6">
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
          ...
          <Button
            ...
            disabled={
              isSaving ||
              (!isActive && !form.api_key) ||
              !form.model_conversation ||
              !form.model_classifier ||
              !form.model_embeddings
            }
          >
