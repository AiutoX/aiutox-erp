...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_tasks_tenant_user_visibility ON tasks (tenant_id, created_by_id, assigned_to_id)"
    )

    # Índice para task assignments lookup
    # Optimiza: JOIN task_assignments WHERE task_id = ? AND tenant_id = ? AND assigned_to_id = ?
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_task_assignments_lookup ON task_assignments (task_id, tenant_id, assigned_to_id)"
    )

    # Índice para filtrar por tenant y user_id en task_assignments
    # Optimiza queries de asignaciones por usuario
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_task_assignments_tenant_user ON task_assignments (tenant_id, assigned_to_id)"
    )
