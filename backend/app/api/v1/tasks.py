...
async def list_tasks(
    ...
    tasks = service.repository.get_all_tasks(
        tenant_id=current_user.tenant_id,
        status=status,
        priority=priority,
...
async def get_tasks_dashboard(
    ...
    async def get_assignments_data():
        ...
        for task_id in task_ids:
            task_assignments = (
                service.repository.get_assignments_by_task(
                    task_id=UUID(str(task_id)), tenant_id=current_user.tenant_id
                )
                if hasattr(service.repository, "get_assignments_by_task")
                else []
    ...
    # Ejecutar todas las consultas en paralelo
    tasks_data: dict | Exception
    settings_data: dict | Exception
    assignments_data: dict | Exception
...
async def get_agenda(
    ...
    # Sort by date
    agenda_items.sort(key=lambda x: x["date"] or "")
    ...
    for line in lines:
        ...
        if line.startswith("BEGIN:VEVENT"):
            ...
            if line.startswith("SUMMARY:"):
                ...
                dt = parse_ics_date(line[8:].strip())
                current_event["dtstart"] = dt.isoformat() if dt else None
            elif line.startswith("DTEND:"):
                dt = parse_ics_date(line[6:].strip())
                current_event["dtend"] = dt.isoformat() if dt else None
