...
import { formatDueDate, dueDateInputValueToIso } from "~/lib/utils/taskDates";
...
const resolveTaskTimes = (task: Task) => {
  ...
  // task.all_day is the single source of truth for whether due_date carries
  // a real time-of-day — a bare ISO datetime string always contains "T"
  // regardless of whether the time component is meaningful, so checking for
  // it (the previous heuristic) was always true and never actually
  // distinguished an all-day due date from a timed one.
  const dueHasTime = Boolean(dueDateRaw) && !task.all_day;
...
const buildTaskUpdatePayload = (
  ...
  if (task.due_date) {
    if (task.all_day) {
      // targetDate is where the user dropped the card, read via local
      // getters (react-big-calendar's own drag handling uses local time) —
      // preserve that calendar day as UTC midnight, not a shifted instant,
      // so an all-day due date never drifts a day when moved.
      const isoDay = dueDateInputValueToIso(
        format(targetDate, "yyyy-MM-dd"),
        true
      );
      return { due_date: isoDay };
    }
...
function MiniCalendar({
  ...
  return (
    <div className="rounded-xl border border-border/60 dark:border-border/40 bg-[hsl(var(--surface))] dark:bg-background p-2.5 space-y-2 shadow-sm">
...
export function TaskCalendar({
  ...
  const canCreateTask = useHasAnyPermission(["tasks.create", "tasks.manage"]);
      ...
      return [
        {
          ...
          metadata: {
            ...
            activity_type:
              (task.start_at && task.end_at && !task.due_date) || task.all_day
                ? "event"
                : "task",
          },
          read_only: false,
  ...
  const handleEventResize = (
    ...
    const isTask =
      event.source_type === "task" || event.metadata?.activity_type === "task";
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-border bg-card dark:bg-[hsl(var(--surface))] dark:border-border/40 px-4 py-3 shadow-sm">
      ...
      <div className="grid gap-6 md:grid-cols-[280px_minmax(0,1fr)] lg:grid-cols-[320px_minmax(0,1fr)]">
        <div className="space-y-4">
          <Card className="border border-border bg-card dark:bg-[hsl(var(--surface))] dark:border-border/40 p-3 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
            ...
            <div className="space-y-2">
              ...
              {dueOnlyTasks.slice(0, 5).map((task) => (
                <button
                  key={task.id}
                  type="button"
                  className="w-full rounded-lg border dark:border-border/40 dark:bg-background px-3 py-2 text-left transition hover:border-primary/60 hover:bg-primary/5 dark:hover:bg-primary/10"
            ...
            <div className="space-y-2">
              ...
              {upcomingTasks.map((task) => (
                <button
                  key={task.id}
                  type="button"
                  className="w-full text-left rounded-lg border dark:border-border/40 dark:bg-background px-3 py-2 hover:border-primary/60 hover:bg-primary/5 dark:hover:bg-primary/10"
                  onClick={() => handleTaskClick(task)}
                >
                  <p className="text-sm font-medium text-foreground">
                    {task.title}
        ...
        <div className="space-y-6">
          {isLoading ? (
            ...
            <div
              className="rounded-2xl border border-border bg-card dark:bg-[hsl(var(--surface))] dark:border-border/40 flex flex-col overflow-hidden shadow-[0_12px_40px_rgba(15,23,42,0.08)]"
              style={{
            ...
            >
              ...
              <div className="border-t border-border/50 dark:border-border/40 bg-[hsl(var(--surface))] dark:bg-background px-6 py-4 flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-foreground">
