...
export async function createImportJob(
  ...
  const response = await apiClient.post<StandardResponse<ImportJobResponse>>(
    "/import-export/import/jobs",
    data
  );
...
export async function listImportJobs(params?: {
  ...
  const response =
    await apiClient.get<StandardListResponse<ImportJobResponse>>(url);
  return response.data;
}
...
export async function createExportJob(
  ...
  const response = await apiClient.post<StandardResponse<ExportJobResponse>>(
    "/import-export/export/jobs",
    data
  );
  return response.data;
}
...
export async function listExportJobs(params?: {
  ...
  const response =
    await apiClient.get<StandardListResponse<ExportJobResponse>>(url);
  return response.data;
}
 ...
 * GET /api/v1/import-export/import/jobs/{job_id}/errors
 */
export async function getImportJobErrors(
  ...
  >(`/import-export/import/jobs/${jobId}/errors`);
  return response.data;
}

/**
 * Download the generated export file
 * GET /api/v1/import-export/export/jobs/{job_id}/download
 */
export async function downloadExportFile(jobId: string): Promise<Blob> {
  const response = await apiClient.get(
    `/import-export/export/jobs/${jobId}/download`,
    { responseType: "blob" }
  );
  return response.data;
}
 ...
 * POST /api/v1/import-export/import/upload
 */
export async function uploadImportFile(
  file: File
): Promise<StandardResponse<{ id: string; name: string; url: string; storage_path: string }>> {
  const formData = new FormData();
  formData.append("file", file);
  const response = await apiClient.post<
    StandardResponse<{ id: string; name: string; url: string; storage_path: string }>
  >("/import-export/import/upload", formData, {
...
export async function listImportTemplates(params?: {
  ...
  const response =
    await apiClient.get<StandardListResponse<ImportTemplateResponse>>(url);
  return response.data;
}
