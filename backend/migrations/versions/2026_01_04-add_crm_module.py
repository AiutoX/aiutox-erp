...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_crm_pipelines_tenant_id", "crm_pipelines", ["tenant_id"], unique=False
    )
    op.create_index(
        "idx_crm_pipelines_tenant_name",
        "crm_pipelines",
        ["tenant_id", "name"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["pipeline_id"], ["crm_pipelines.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
    ...
    op.create_index(
        "ix_crm_leads_pipeline_id", "crm_leads", ["pipeline_id"], unique=False
    )
    op.create_index(
        "ix_crm_leads_organization_id", "crm_leads", ["organization_id"], unique=False
    )
    op.create_index(
        "ix_crm_leads_contact_id", "crm_leads", ["contact_id"], unique=False
    )
    op.create_index("ix_crm_leads_status", "crm_leads", ["status"], unique=False)
    op.create_index(
        "idx_crm_leads_tenant_status",
        "crm_leads",
        ["tenant_id", "status"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column("probability", sa.Numeric(precision=5, scale=2), nullable=True),
        sa.Column(
            "expected_close_date", postgresql.TIMESTAMP(timezone=True), nullable=True
        ),
        sa.Column("assigned_to_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("created_by_id", postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column("next_event_id", postgresql.UUID(as_uuid=True), nullable=True),
    ...
    op.create_index(
        "ix_crm_opportunities_tenant_id",
        "crm_opportunities",
        ["tenant_id"],
        unique=False,
    )
    op.create_index(
        "ix_crm_opportunities_pipeline_id",
        "crm_opportunities",
        ["pipeline_id"],
        unique=False,
    )
    op.create_index(
        "ix_crm_opportunities_organization_id",
        "crm_opportunities",
        ["organization_id"],
        unique=False,
    )
    op.create_index(
        "ix_crm_opportunities_contact_id",
        "crm_opportunities",
        ["contact_id"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_crm_opportunities_organization_id", table_name="crm_opportunities"
    )
