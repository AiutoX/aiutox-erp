...
export function TaskStatusManagerModal({
  ...
  const canManageStatuses = useHasAnyPermission(["tasks.manage"]);
  const {
    statuses,
    createStatus,
    updateStatus,
  ...
  const isNameTaken = useMemo(() => {
    ...
    return trimmedName
      ? isStatusNameTaken(trimmedName, editingStatus?.id)
      : false;
          ...
          <div className="flex gap-2">
            <Button
              ...
              disabled={
                ...
                isUpdating ||
                !formData.name.trim() ||
                isStatusNameTaken(formData.name.trim(), editingStatus?.id)
              }
            >
  ...
  const content = (
    <div className="space-y-6">
      ...
      {canManageStatuses && (isCreatingNew || editingStatus) && <StatusForm />}

      <div className="space-y-8">
        <StatusList
          statuses={getCustomStatuses()}
          title={t("tasks.statusManager.custom")}
          showActions={canManageStatuses}
        />
        <StatusList
