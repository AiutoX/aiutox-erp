...
class RedisStreamsClient:
    ...
    async def _get_client(self) -> aioredis.Redis:
        ...
        if self._client is None:
            try:
                ...
                # If auth fails and we tried with a password, retry without it
                if "AUTH" in str(e) and self.password and "@" not in self.redis_url:
                    logger.warning(
                        f"Auth failed with password, retrying without auth: {e}"
                    )
                    try:
                        self._client = aioredis.from_url(
                            self.redis_url,
                        ...
                        await self._client.ping()  # type: ignore[misc, union-attr]
                        logger.info("Connected to Redis successfully (without auth)")
                    except (ConnectionError, RedisError) as retry_error:
                        logger.error(f"Failed to connect to Redis: {retry_error}")
                        raise PubSubError(
                            f"Failed to connect to Redis: {retry_error}"
                        ) from retry_error
                elif isinstance(e, (ConnectionError, RedisError)):
    ...
    async def publish(
        ...
        Args:
            stream: Stream name (e.g., 'events:comments')
            event: Event type string (e.g., 'comment.created')
        ...
        async with self.connection() as client:
            ...
            message_id: str = await client.xadd(stream, message)
            logger.debug(
                f"Published event '{event}' to stream '{stream}' (ID: {message_id})"
            )
            return message_id
    ...
    async def add_message(
        ...
        """Add a raw message to a Redis stream.
        ...
        """
        async with self.connection() as client:
            message_id: str = await client.xadd(stream, data)
            return message_id
