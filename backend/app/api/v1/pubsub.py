...
def _entry_id(entry: Any, default: str | None = "0-0") -> str | None:
    """Extract the message ID from a redis-py XINFO STREAM entry.

    redis-py returns first-entry/last-entry as (id, fields_dict) tuples, not
    a bare ID string, despite the field name — callers only ever want the ID.
    """
        ...
        for stream_name in [
            ...
            try:
                ...
                stats["streams"][stream_name] = {
                    ...
                    "groups": groups_info,
                    "first_entry_id": _entry_id(stream_info.get("first-entry")),
                    "last_entry_id": _entry_id(stream_info.get("last-entry")),
                }
            except PubSubError:
