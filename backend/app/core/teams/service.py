...
class TeamService:
        ...
        if include_nested:
            ...
            for child_team in child_teams:
                child_members = self.get_group_members(
                    tenant_id, UUID(str(child_team.id)), include_nested=True
                )
                user_ids.extend(child_members)  # type: ignore[arg-type]

        # Eliminar duplicados
        return list(set(user_ids))  # type: ignore[arg-type]
    ...
    def get_user_groups(
        ...
        if include_parent_groups:
            for group_id in list(group_ids):
                parent_teams = self._get_parent_teams(tenant_id, UUID(str(group_id)))
                group_ids.extend([team.id for team in parent_teams])  # type: ignore[arg-type]

        # Eliminar duplicados
        return list(set(group_ids))  # type: ignore[arg-type]
    ...
    def _get_parent_teams(self, tenant_id: UUID, team_id: UUID) -> list[Team]:
        ...
        grandparents = self._get_parent_teams(tenant_id, UUID(str(parent.id)))
        ...
        if include_nested:
            ...
            for child_team in child_teams:
                if self.is_user_in_group(
                    tenant_id, user_id, UUID(str(child_team.id)), include_nested=True
                ):
                    return True
