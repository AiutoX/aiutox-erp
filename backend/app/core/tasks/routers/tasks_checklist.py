...
async def update_checklist_item(
    ...
    item = service.get_checklist_item(item_id, current_user.tenant_id)
    if not item:
        raise APIException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    task = service.get_task(item.task_id, current_user.tenant_id)
    if not task:
        raise APIException(
            status_code=status.HTTP_404_NOT_FOUND,
            code="TASK_NOT_FOUND",
            message=f"Task with ID {item.task_id} not found",
        )
