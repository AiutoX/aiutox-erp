...
import { formatDueDate } from "~/lib/utils/taskDates";
...
export const TaskAdvancedFilter = memo(
  ({
    ...
    const formatDate = (dateString: string, allDay: boolean) => {
      return formatDueDate(dateString, allDay, "PPP", dateLocale);
    };
    ...
    const formatStartEndDates = (
      ...
      const formatDate = (dateStr: string) =>
        formatDueDate(dateStr, Boolean(allDay), "d MMM yyyy", dateLocale);
      ...
      },
      {
        ...
        cell: (task: Task) => {
          ...
          if (activityType === "evento") {
            ...
            const timeRemaining = task.end_at
              ? getTimeRemaining(task.end_at)
              : "Sin fecha";
            return (
              <Badge
                variant="outline"
        ...
        cell: (task: Task) => {
          ...
          return (
            <div className="flex flex-wrap gap-1">
              {taskAssignments.map(
                (assignment: { id: string; userName: string }) => (
                  <Badge
                    key={assignment.id}
                    variant="secondary"
                    className="text-xs"
      ...
      },
      {
        ...
        cell: (task: Task) => {
          ...
          if (activityType === "evento") {
            ...
            const dates = formatStartEndDates(
              task.start_at,
              task.end_at,
              task.all_day
            );
            ...
            return (
              <span className="text-sm">
                {task.due_date
                  ? formatDate(task.due_date, Boolean(task.all_day))
                  : "-"}
              </span>
            );
          }
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                {filters.status && (
                  <Badge variant="outline" className="flex items-center gap-1">
                    Estado:{" "}
                    {t(`tasks.statuses.${filters.status}`) || filters.status}
                    <Button
                      variant="ghost"
                      size="icon"
