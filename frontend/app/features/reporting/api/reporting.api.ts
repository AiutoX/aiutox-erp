...
/**
 * List report execution history (Auditoria tab)
 * GET /api/v1/reporting/reports/executions
 *
 * Requires: auth.view_audit permission
 */
export async function listReportExecutions(
  params?: ReportExecutionHistoryParams
): Promise<StandardListResponse<ReportExecutionHistory>> {
  const response = await apiClient.get<StandardListResponse<ReportExecutionHistory>>(
    "/reporting/reports/executions",
    {
      params: {
        report_id: params?.report_id || undefined,
