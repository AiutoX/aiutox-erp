...
class PaginationParams(BaseModel):
    ...
    sort_order: str = Field(
        default="desc", pattern="^(asc|desc)$", description="Orden (asc/desc)"
    )
...
class PaginatedResponse[T](BaseModel):
    ...
    def create(
        ...
        return cls(data=data, meta=meta, links=links)  # type: ignore[arg-type]
...
class CursorPaginationParams(BaseModel):
    ...
    sort_order: str = Field(default="desc", pattern="^(asc|desc)$", description="Orden")
...
class TaskQueryOptimizer:
    ...
    def optimize_task_list_query(
        ...
        if assigned_to_filter:
            query = query.filter(Task.assigned_to_id == assigned_to_filter)
...
class PaginationCacheManager:
    ...
    def generate_cache_key(
        ...
        param_hash = hashlib.md5(
            param_string.encode(), usedforsecurity=False
        ).hexdigest()
