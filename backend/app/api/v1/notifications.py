...
@router.post(
    ...
    description="Create a new notification template. Requires notifications.write permission.",
)
async def create_template(
    template_data: NotificationTemplateCreate,
    current_user: Annotated[User, Depends(require_permission("notifications.write"))],
...
@router.get(
    ...
    description="List all notification templates for the current tenant. Requires notifications.read permission.",
)
async def list_templates(
    current_user: Annotated[User, Depends(require_permission("notifications.read"))],
    repository: Annotated[NotificationRepository, Depends(get_notification_repository)],
...
@router.get(
    ...
    description="Get a specific notification template by ID. Requires notifications.read permission.",
)
...
@router.put(
    ...
    description="Update a notification template. Requires notifications.write permission.",
)
...
@router.post(
    ...
    description="Send a notification manually. Requires notifications.admin permission.",
)
async def send_notification(
    request: NotificationSendRequest,
    current_user: Annotated[User, Depends(require_permission("notifications.admin"))],
...
@router.get(
    ...
    description="List notification queue entries for the current tenant. Requires notifications.read permission.",
)
            ...
            while True:
                try:
                    ...
                    new_notifications = repository.get_unread_notifications(
                        ...
                        since_id=UUID(str(last_id)) if last_id is not None else None,
                        limit=50,
                    )
                    ...
                    # Sleep with current interval first — catch CancelledError to exit cleanly
                    try:
