...
export const ConditionNode = memo(
  ({ data, selected }: NodeProps<ConditionNodeData>) => {
    const opLabel = data.negate
      ? `NOT ${OPERATOR_LABEL[data.operator] ?? data.operator}`
      : (OPERATOR_LABEL[data.operator] ?? data.operator);
    ...
    return (
      <div
        className={`px-4 py-3 rounded-lg border-2 bg-card shadow-lg min-w-50 ${
          selected
          ...
          <div className="flex-1 min-w-0">
            ...
            {data.field && (
              <div className="flex items-center gap-1 mt-1 flex-wrap">
                <span className="text-xs bg-muted text-muted-foreground px-1.5 py-0.5 rounded font-mono truncate max-w-20">
                  {data.field}
                </span>
                <span className="text-xs text-amber-700 dark:text-amber-300 font-semibold">
                  {opLabel}
                </span>
                <span className="text-xs bg-muted text-muted-foreground px-1.5 py-0.5 rounded truncate max-w-20">
                  {valueStr}
                </span>
              </div>
            )}
