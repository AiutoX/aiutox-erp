...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_approval_flows_updated_by", "approval_flows", ["updated_by"], unique=False
    )
    op.create_index(
        "ix_approval_flows_deleted_at", "approval_flows", ["deleted_at"], unique=False
    )
    ...
    op.create_index(
        "ix_approval_actions_ip_address",
        "approval_actions",
        ["ip_address"],
        unique=False,
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "fk_approval_flows_updated_by_users", "approval_flows", type_="foreignkey"
    )
