...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_task_statuses_tenant_id", "task_statuses", ["tenant_id"], unique=False
    )
    op.create_index(
        "idx_task_statuses_tenant_order",
        "task_statuses",
        ["tenant_id", "order"],
        unique=False,
    )
    op.create_index(
        "idx_task_statuses_tenant_type",
        "task_statuses",
        ["tenant_id", "type"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "checklist_items", postgresql.JSONB(astext_type=sa.Text()), nullable=True
        ),
        ...
        sa.ForeignKeyConstraint(
            ["default_status_id"], ["task_statuses.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(["created_by_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        "ix_task_templates_tenant_id", "task_templates", ["tenant_id"], unique=False
    )
    op.create_index(
        "idx_task_templates_tenant_name",
        "task_templates",
        ["tenant_id", "name"],
        unique=False,
    )
    ...
    op.add_column(
        "tasks", sa.Column("status_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
    op.add_column("tasks", sa.Column("board_order", sa.Integer(), nullable=True))
    op.add_column(
        "tasks", sa.Column("template_id", postgresql.UUID(as_uuid=True), nullable=True)
    )
    ...
    op.create_index(
        "idx_tasks_board_order", "tasks", ["status_id", "board_order"], unique=False
    )
