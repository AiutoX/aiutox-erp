...
from app.core.gamification.models import GamificationEvent
...
class GamificationEventHandler:
    ...
    def __init__(
        ...
        self.config_service = ConfigService(db, use_cache=True)
    ...
    def handle_task_completed(
        ...
        try:
            ...
            logger.info(
                f"task.completed: user={user_id}, task={task_id}, points={points}"
            )
    ...
    def _calculate_points(
        ...
        """
        # Base points from tenant config, falling back to hardcoded defaults.
        module_config = self.config.get(source_module, {})
        default_base = module_config.get(event_type, 10)
        base_points = self._get_config_value(
            "points_per_task" if event_type == "task.completed" else None,
            default_base,
        )

        # Priority bonus (only for task events) — tenant-configurable per priority.
        priority = metadata.get("priority", "medium")
        default_priority_bonus = PRIORITY_BONUS.get(priority, 0)
        base_points += self._get_config_value(
            f"priority_bonus_{priority}", default_priority_bonus
        )

        # On-time completion bonus
        if metadata.get("completed_on_time"):
            base_points += self._get_config_value("on_time_bonus", ON_TIME_BONUS)

        # Streak bonus
        user_points = self.points_service.get_user_points(
            metadata.get("user_id", UUID(int=0))
        )
        streak_threshold = self._get_config_value(
            "streak_bonus_threshold", STREAK_BONUS_THRESHOLD
        )
        if user_points and user_points.current_streak >= streak_threshold:
            base_points += self._get_config_value(
    ...
    def _get_config_value(self, key: str | None, default: int) -> int:
        """Read a tenant-configured gamification points value, falling back
        to the hardcoded default when no key is given or nothing is saved."""
        if key is None:
            return default
