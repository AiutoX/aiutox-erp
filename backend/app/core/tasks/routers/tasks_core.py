...
from aiutox_sdk.response import PaginationMeta, StandardListResponse, StandardResponse
...
from app.core.tasks.ownership import user_can_delete_task, user_owns_task
...
TASK_SETTINGS_KEYS = {
    ...
    "comments_attachments_enabled": "comments.attachments_enabled",
}
...
def _build_task_settings(
    ...
    return TaskModuleSettings(
        calendar_enabled=config.get("calendar.enabled", True),
        ...
        comments_attachments_enabled=config.get("comments.attachments_enabled", False),
    )
...
async def list_tasks(
    ...
    parent_task_id: UUID | None = Query(
        default=None, description="Filter by parent task (subtasks)"
    ),
...
@router.put(
    ...
    status_code=status.HTTP_200_OK,
    summary="Update task",
    description="Update a task. Requires tasks.manage permission and ownership.",
)
...
async def update_task(
    ...
    task = await service.update_task(
        task_id, current_user.tenant_id, current_user.id, **update_dict
    )
...
@router.delete(
    ...
    summary="Delete task",
    description="Delete a task. Requires tasks.manage permission and ownership (creator only).",
)
...
async def delete_task(
    ...
    if not user_can_delete_task(task, current_user.id, user_permissions):
        raise_forbidden(
            code="AUTH_INSUFFICIENT_PERMISSIONS",
            message="Insufficient permissions",
            details={"required_permission": "tasks.manage.all"},
