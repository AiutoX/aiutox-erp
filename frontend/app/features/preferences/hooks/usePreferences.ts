...
/**
 * Fetch notification event types available for preference configuration,
 * self-discovered from all enabled modules via the backend registry.
 */
export function useNotificationEventTypes() {
  return useQuery({
    queryKey: ["preferences", "notification-event-types"],
    queryFn: async () => {
      const response = await getNotificationEventTypes();
