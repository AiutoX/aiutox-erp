...
def _auto_register_builtin_sources(registry: DataSourceRegistry) -> None:
    ...
    try:
        from app.core.reporting.sources.products_data_source import (
            ProductsDataSource,
            _product_repository_available,
        )

        # The wrapper module itself always imports cleanly — it swallows
        # the real ImportError from aiutox_module_products internally and
        # exposes it only via this flag (UX-004b). Checking the module
        # import alone would always register "products", even when the
        # package isn't installed.
        if _product_repository_available:
