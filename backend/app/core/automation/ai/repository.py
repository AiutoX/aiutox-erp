...
class AIRepository:
    ...
    def upsert_provider_config(
        ...
        *,
        ...
        # A tenant has exactly one active provider config at a time — deactivate
        # any other provider_type row still marked active before (re)activating
        # this one, otherwise get_active_provider_config's unordered query can
        # return an arbitrary row when more than one is is_active=True.
        self.db.query(AILLMProviderConfig).filter(
            AILLMProviderConfig.tenant_id == tenant_id,
            AILLMProviderConfig.provider_type != provider_type,
            AILLMProviderConfig.is_active == True,  # noqa: E712
