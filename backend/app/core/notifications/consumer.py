...
class NotificationEventConsumer:
    ...
    async def _handle_event(self, event: Event) -> None:
        ...
        try:
            ...
            # --- Sprint 5: Billing / Lease / Maintenance notifications ---
            elif event_type == "billing.cobro_generado":
                await self._handle_billing_event(event, ["whatsapp", "email"])
                return
            elif event_type == "billing.pago_recibido":
                await self._handle_billing_event(event, ["whatsapp", "email"])
                return
            elif event_type == "billing.aviso_mora":
                await self._handle_billing_event(event, ["whatsapp"])
                return
            elif event_type == "billing.intereses_mora":
                await self._handle_billing_event(event, ["whatsapp", "email"])
                return
            elif event_type == "lease.contrato_vence":
                await self._handle_lease_event(event, ["email"])
                return
            elif event_type == "lease.notif_reajuste":
                await self._handle_lease_event(event, ["email", "whatsapp"])
                return
            elif event_type == "maintenance.ot_asignada":
                await self._handle_maintenance_event(event, ["whatsapp"])
                return
            elif event_type == "maintenance.presupuesto_aprobado":
                await self._handle_maintenance_event(event, ["whatsapp", "email"])
                return

            # --- Issue 011: Material request notification routing ---
            elif event_type.startswith("maintenance.material_request."):
                await self._handle_material_request_event(event)
                return
    ...
    async def _handle_billing_event(self, event: Event, channels: list[str]) -> None:
        """Handle billing domain events (cobro, pago, mora, intereses)."""
        if not event.user_id:
            logger.debug("No user_id in billing event %s, skipping", event.event_type)
            return
        data = self._extract_notification_data(event)
        data["tenant_id"] = str(event.tenant_id)
        await self.notification_service.send(
            event_type=event.event_type,
            recipient_id=event.user_id,
            channels=channels,
        ...
        logger.info("Sent %s notification (channels: %s)", event.event_type, channels)

    async def _handle_lease_event(self, event: Event, channels: list[str]) -> None:
        """Handle lease domain events (contrato_vence, notif_reajuste)."""
        if not event.user_id:
            logger.debug("No user_id in lease event %s, skipping", event.event_type)
            return
        data = self._extract_notification_data(event)
        data["tenant_id"] = str(event.tenant_id)
        await self.notification_service.send(
    ...
    async def _handle_maintenance_event(
        self, event: Event, channels: list[str]
    ) -> None:
        """Handle maintenance domain events (ot_asignada, presupuesto_aprobado)."""
        metadata = event.metadata.additional_data if event.metadata else {}
        # Prefer assigned_to_id for maintenance events; fallback to event.user_id
        recipient_raw = metadata.get("assigned_to_id") or (
            str(event.user_id) if event.user_id else None
        )
        if not recipient_raw:
            logger.debug(
                "No recipient in maintenance event %s, skipping", event.event_type
            )
            return
        data = self._extract_notification_data(event)
        data["tenant_id"] = str(event.tenant_id)
        await self.notification_service.send(
            event_type=event.event_type,
            recipient_id=UUID(recipient_raw),
            channels=channels,
            data=data,
            tenant_id=event.tenant_id,
        )
    ...
    async def _handle_material_request_event(self, event: Event) -> None:
        """Handle material_request.* events using NotificationRoutingService.

        Routes notifications based on notification rule templates, property overrides,
        and user preferences. Falls back to hardcoded defaults if no template matches.
        """
        from app.core.notifications.routing import NotificationRoutingService

        try:
            routing_service = NotificationRoutingService(self.db)

            # Extract context from event data
            data = event.metadata.additional_data if event.metadata else {}
            property_id = data.get("property_id")
            context_type = "property" if property_id else "tenant"
            context_id = UUID(property_id) if property_id else event.tenant_id

            await routing_service.route_and_send(
                event_type=event.event_type,
            ...
            logger.info(
                f"Material request notification routed for event {event.event_type}"
            )
        except Exception as e:
            logger.error(
                f"Error routing material request event {event.event_type}: {e}",
                exc_info=True,
            )
            # Don't re-raise — we want to continue processing other events
