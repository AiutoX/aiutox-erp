"""Products data source for reporting — optional, requires aiutox-module-products installed."""

from typing import Any
from uuid import UUID

from sqlalchemy.orm import Session

from app.core.reporting.data_source import BaseDataSource

try:
    from aiutox_module_products.repositories.product_repository import (
        ProductRepository as _ProductRepository,
    )

    _product_repository_available = True
except ImportError:
    _ProductRepository = None  # type: ignore[assignment,misc]
    _product_repository_available = False


class ProductsDataSource(BaseDataSource):
    """Data source for products reporting."""

    required_permission = "products.view"
    dataset_type = "products"

    def __init__(
        self,
        db: Session,
        tenant_id: UUID,
        user_permissions: set[str] | None = None,
    ):
        """Initialize products data source.

        Args:
            db: Database session
            tenant_id: Tenant ID
            user_permissions: Effective permission set of the requesting user
        """
        super().__init__(db, tenant_id, user_permissions)
        if not _product_repository_available:
            raise ImportError("aiutox-module-products is not installed")
        self.product_repo = _ProductRepository(db)  # type: ignore[call-arg]

    async def get_data(
        self,
        filters: dict[str, Any] | None = None,
        pagination: dict[str, int] | None = None,
    ) -> dict[str, Any]:
        """Get products data.

        Args:
            filters: Filter configuration
            pagination: Pagination configuration

        Returns:
            Dictionary with 'data' and 'total'
        """
        skip = pagination.get("skip", 0) if pagination else 0
        limit = pagination.get("limit", 100) if pagination else 100

        # Apply filters
        category_id = filters.get("category_id") if filters else None
        search = filters.get("search") if filters else None

        # Get products based on filters
        if category_id:
            products = self.product_repo.get_all_by_category(
                tenant_id=self.tenant_id,
                category_id=category_id,
                skip=skip,
                limit=limit,
            )
        elif search:
            products = self.product_repo.search(
                tenant_id=self.tenant_id, query=search, skip=skip, limit=limit
            )
        else:
            products = self.product_repo.get_all_by_tenant(
                tenant_id=self.tenant_id, skip=skip, limit=limit
            )

        # Get total count
        total = self.product_repo.count_by_tenant(self.tenant_id)

        # Convert to dictionaries
        data = [
            {
                "id": str(product.id),
                "sku": product.sku,
                "name": product.name,
                "price": float(product.price) if product.price else None,
                "cost": float(product.cost) if product.cost else None,
                "is_active": product.is_active,
            }
            for product in products
        ]
        data = self._filter_rows(data)

        return {"data": data, "total": total}

    def get_columns(self) -> list[dict[str, Any]]:
        """Get available columns for products.

        Returns:
            List of column definitions
        """
        columns = [
            {
                "name": "id",
                "type": "uuid",
                "label": "ID",
                "label_key": "reporting.columns.products.id",
            },
            {
                "name": "sku",
                "type": "string",
                "label": "SKU",
                "label_key": "reporting.columns.products.sku",
            },
            {
                "name": "name",
                "type": "string",
                "label": "Name",
                "label_key": "reporting.columns.products.name",
            },
            {
                "name": "price",
                "type": "decimal",
                "label": "Price",
                "label_key": "reporting.columns.products.price",
            },
            {
                "name": "cost",
                "type": "decimal",
                "label": "Cost",
                "label_key": "reporting.columns.products.cost",
            },
            {
                "name": "is_active",
                "type": "boolean",
                "label": "Active",
                "label_key": "reporting.columns.products.is_active",
            },
        ]
        return self._filter_columns(columns)

    def get_filters(self) -> list[dict[str, Any]]:
        """Get available filters for products.

        Returns:
            List of filter definitions
        """
        return [
            {
                "name": "category_id",
                "type": "uuid",
                "label": "Category",
                "label_key": "reporting.columns.products.category_id",
                "options": None,  # Could be populated with actual categories
            },
            {
                "name": "search",
                "type": "string",
                "label": "Search",
                "label_key": "reporting.columns.products.search",
                "options": None,
            },
            {
                "name": "is_active",
                "type": "boolean",
                "label": "Active",
                "label_key": "reporting.columns.products.is_active",
                "options": None,
            },
        ]
