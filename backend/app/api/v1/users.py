...
@router.get(
    ...
    summary="List users",
    description=(
        "List users in the current tenant with optional filters. Requires "
        "users.view (directory-style, restricted fields) or auth.manage_users "
        "(full user management, all fields)."
    ),
    responses={
                ...
                "application/json": {
                    "example": {
                        "error": {
                            ...
                            "details": {"required_permission": "users.view"},
                        }
                    }
                }
            },
...
async def list_users(
    ...
    # Callers with only the directory-style users.view permission (not full
    # auth.manage_users) get a restricted, name/email-only shape — no role,
    # status, or timestamps.
    if not has_permission(user_permissions, "auth.manage_users"):
        directory_fields = {"id", "email", "full_name", "first_name", "last_name"}
        users = [
            {key: value for key, value in user.items() if key in directory_fields}
            for user in users
        ]
...
async def create_user(
    ...
    try:
        ...
        return StandardResponse(data=user)  # type: ignore[arg-type]
