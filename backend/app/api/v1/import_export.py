...
@router.get(
    ...
    summary="List modules that support import/export",
    description=(
        "Returns only enabled modules that actually implement "
        "ImportExportHandler, not every enabled module. Requires "
        "import_export.view permission."
    ),
)
async def get_supported_modules(
    ...
    """List enabled modules that implement ImportExportHandler."""
    from app.core.module_registry import get_module_registry

    registry = get_module_registry()
    modules = registry.get_import_export_supported_modules(current_user.tenant_id)

    return StandardResponse(
        data=modules,
        message="Supported modules retrieved successfully",
    )
...
async def create_import_job(
    ...
    """Create a new import job and process it synchronously."""
    job = service.create_import_job(
        job_data=job_data.model_dump(exclude_none=True),
        tenant_id=current_user.tenant_id,
        user_id=current_user.id,
    )
    job = await service.process_import_job(
        UUID(str(job.id)), current_user.tenant_id, current_user.id
    )
...
async def create_export_job(
    ...
    """Create a new export job and process it synchronously."""
    job = service.create_export_job(
        job_data=job_data.model_dump(exclude_none=True),
        tenant_id=current_user.tenant_id,
        user_id=current_user.id,
    )
    job = await service.process_export_job(
        UUID(str(job.id)), current_user.tenant_id, current_user.id
    )
