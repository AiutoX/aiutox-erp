...
import { humanizeActivityType } from "~/features/activities/utils/activityTypes";
...
export function ActivityTimeline({
  ...
  const getActivityTypeLabel = (type: ActivityType) => {
    ...
    const label = t(key);
    // useTranslation() returns the literal key on a miss (not a falsy value),
    // so a raw, unrecognized activity_type (e.g. custom types emitted by
    // business modules like "lease.activo") would otherwise render the
    // untranslated i18n key itself.
    return label === key ? humanizeActivityType(type) : label;
  };
