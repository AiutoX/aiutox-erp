...
import { Mail, Phone, Eye, Pencil, Power, Plus, Search, Filter, Send } from "lucide-react";
...
import { SendMessageDialog } from "~/features/crm/components/SendMessageDialog";
...
interface ContactsListProps {
  ...
  openSendMessage: (c: Contact) => void;
        ...
        return (
          ...
          >
            ...
            <div className="flex gap-1 shrink-0">
              ...
              {canSendMessage && contact.is_active && (
                <Button
                  variant="outline"
                  size="icon"
                  className="h-8 w-8"
                  onClick={() => openSendMessage(contact)}
                  title={t("contacts.actions.send_message")}
                >
...
export default function ContactsPage() {
  ...
  const canSendMessage = hasPermission("crm.edit");
  ...
  const [messageMethods, setMessageMethods] = useState<ContactMethod[]>([]);
  ...
  async function openSendMessage(contact: Contact) {
    setMessageContact(contact);
    setMessageMethods([]);
    try {
      const res = await getContactMethods("contact", contact.id);
      setMessageMethods(res.data ?? []);
    } catch {
      // dialog will show "no channels available" if methods failed to load
    }
  }
