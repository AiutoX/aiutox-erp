...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "key", sa.String(length=255), nullable=False, comment="Configuration key"
        ),
        sa.Column(
            "value",
    ...
    op.create_index(
        "ix_system_configs_tenant_id", "system_configs", ["tenant_id"], unique=False
    )
    op.create_index(
        "ix_system_configs_module", "system_configs", ["module"], unique=False
    )

    # Composite index for common queries
    op.create_index(
        "idx_system_configs_tenant_module",
        "system_configs",
        ["tenant_id", "module"],
        unique=False,
    )
