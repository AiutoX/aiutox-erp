...
down_revision: str | None = "a8ae578ac76b"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.ForeignKey("system_configs.id", ondelete="CASCADE"),
            nullable=False,
    ...
    op.create_index(
        "ix_config_versions_config_id", "config_versions", ["config_id"], unique=False
    )
    op.create_index(
        "ix_config_versions_tenant_id", "config_versions", ["tenant_id"], unique=False
    )
    op.create_index(
        "ix_config_versions_module", "config_versions", ["module"], unique=False
    )
    op.create_index("ix_config_versions_key", "config_versions", ["key"], unique=False)
    op.create_index(
        "ix_config_versions_created_at", "config_versions", ["created_at"], unique=False
    )

    # Composite indexes
    op.create_index(
        "ix_config_versions_tenant_module",
        "config_versions",
        ["tenant_id", "module"],
        unique=False,
    )
    op.create_index(
        "ix_config_versions_config_version",
        "config_versions",
        ["config_id", "version_number"],
        unique=False,
    )
    op.create_index(
        "ix_config_versions_tenant_module_key",
        "config_versions",
        ["tenant_id", "module", "key"],
        unique=False,
    )
