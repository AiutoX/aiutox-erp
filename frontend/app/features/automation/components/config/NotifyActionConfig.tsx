...
import { useNotificationChannelsCatalog } from "~/features/notifications/hooks/useNotifications";
...
export function NotifyActionConfig({ data, onChange }: NotifyActionConfigProps) {
  ...
  const channels_catalog = catalogResp?.data ?? [];
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex flex-col gap-1.5">
          {channels_catalog.map((entry) => (
            ...
            >
              <Checkbox
                checked={channels.includes(entry.channel)}
                disabled={!entry.available}
                onCheckedChange={() => toggleChannel(entry.channel)}
              />
              <span className="capitalize">{entry.channel}</span>
              {!entry.available && (
                <span className="text-xs text-muted-foreground">
                  ({t("automation.nodeConfig.notify.channelUnavailable")})
                </span>
              )}
            </label>
          ))}
