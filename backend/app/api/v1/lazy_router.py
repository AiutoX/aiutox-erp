...
def get_api_router() -> APIRouter:
    ...
    # Canonical automation router (BUG-002): the core-module router is the
    # single automation HTTP surface; app.api.v1.automation was retired.
    from app.core.automation.router import router as automation_router
    ...
    try:
        from app.core.gamification.api import router as gamification_router
    ...
    _api_router.include_router(search_router, prefix="/search", tags=["search"])
    _api_router.include_router(
        integrations.router, prefix="/integrations", tags=["integrations"]
    )
    _api_router.include_router(calendar.router, prefix="/calendar", tags=["calendar"])
    _api_router.include_router(
        channel_identities.router,
    ...
    if billing.router is not None:
        _api_router.include_router(billing.router, tags=["billing"])
    if finances.router is not None:
        _api_router.include_router(finances.router, tags=["finances"])
    ...
    # NOTE: the dead SearchRegistry-based registration (register_searchable_entities())
    # was removed here in SRC-001 — its registrations were never consulted by
    # the real search query path. Real search providers self-register via
    # ModuleInterface.get_search_handler(), discovered by ModuleRegistry, no
    # explicit call needed here (see docs/04-modules/core/search/MODULE-SPEC.md).
