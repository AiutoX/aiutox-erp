...
class EvolutionApiProvider(NotificationProvider):
    ...
    def __init__(
        ...
        """Initialize Evolution API provider with explicit, per-tenant credentials.

        No global settings are read here — the caller resolves credentials via
        IntegrationService.get_whatsapp_credentials() before constructing this.
        """
    ...
    async def send(
        ...
        # Evolution API expects a bare digit string (country code + number,
        # no leading '+' or formatting) — validate_recipient() above already
        # strips '+' for its own pattern check, but the outbound payload was
        # still sending the raw, unstripped string, causing a real "Invalid
        # number (400)" rejection for any recipient entered in international
        # "+55..." format. Normalize here so both paths agree.
        normalized_recipient = re.sub(r"\D", "", recipient)
