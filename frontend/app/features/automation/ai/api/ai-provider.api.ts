...
export interface ProviderConfigCreate {
  provider_type: ProviderType;
  // null means "keep the stored key unchanged" — matches the backend's
  // `str | None`. Never send "": min_length=10 applies to any string the
  // backend receives, so an empty string fails validation (UX-010).
  api_key: string | null;
  base_url?: string | null;
  model_conversation?: string;
  model_classifier?: string;
