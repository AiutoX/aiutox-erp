...
class Role(Base):
    """A role: either a system role or a tenant-defined custom role.

    System roles (is_system=True, tenant_id=None) are the 5 original
    hardcoded roles (owner, admin, manager, staff, viewer), migrated here
    from the ROLE_PERMISSIONS dict in app/core/auth/permissions.py so all
    roles resolve through a single model. Custom roles (is_system=False)
    are tenant-scoped and created via the /auth/custom-roles endpoints.

    permissions is a flat list of permission strings (e.g. "inventory.view")
    using the same {module}.{action} namespace already used everywhere else
    (see has_permission() in permissions.py) -- no new namespace introduced.
    """
    ...
    def __repr__(self) -> str:
        return (
            ...
            f"is_system={self.is_system}, tenant_id={self.tenant_id})>"
        )
