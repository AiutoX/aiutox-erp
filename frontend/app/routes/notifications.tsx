...
const CHANNEL_LABEL_KEY: Record<string, string> = {
  ...
  "in-app": "notifications.channel.inApp",
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 space-y-2">
            <div className="flex items-center gap-2">
              ...
              >
                {(() => {
                  const key = CHANNEL_LABEL_KEY[notification.channel];
                  return key ? t(key) : notification.channel;
                })()}
              </Badge>
              <Badge variant={STATUS_VARIANT[notification.status] ?? "outline"}>
                {(() => {
                  const key = STATUS_LABEL_KEY[notification.status];
                  return key ? t(key) : notification.status;
                })()}
              </Badge>
            </div>
