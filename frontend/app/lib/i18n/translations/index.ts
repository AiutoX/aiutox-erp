      ...
      if (matches) {
        ...
        if (moduleName && lang) {
          ...
          if (lang === "es" || lang === "en") {
            ...
            moduleBucket[lang] = deepMergeRecords(
              moduleBucket[lang],
              flatToNested(raw)
            );
          }
...
/**
 * Top-level translation keys deliberately reused across multiple modules
 * (e.g. both `tasks` and `views` define `savedFilters`). Merging these is
 * intentional and must not trip collision detection.
 */
...
/**
 * Merges every module's translations into one object, keyed by top-level
 * key. Two different modules defining the same non-shared top-level key
 * (see SHARED_TRANSLATION_KEYS) is a collision — see EH-002, where
 * `integrations` defined a stray root-level `title` that silently clobbered
 * `tasks`'s heading. Fails fast at load time instead of silently picking
 * whichever module's file the glob merges last.
 */
export function mergeModuleTranslationsByLanguage(
  modules: ModuleTranslations,
  language: "es" | "en"
): Record<string, unknown> {
  const keyOwners: Record<string, string> = {};

  return Object.entries(modules).reduce<Record<string, unknown>>(
    (acc, [moduleName, trans]) => {
      const moduleContent = trans[language] ?? {};

      for (const key of Object.keys(moduleContent)) {
        if ((SHARED_TRANSLATION_KEYS as readonly string[]).includes(key)) {
          continue;
        }

        const owner = keyOwners[key];
        if (owner && owner !== moduleName) {
          throw new Error(
            `i18n collision: top-level key "${key}" is defined by both ` +
              `"${owner}" and "${moduleName}". Nest it under the module's ` +
              `own key, or add it to SHARED_TRANSLATION_KEYS if the reuse ` +
              `is intentional.`
          );
        }
        keyOwners[key] = moduleName;
      }

      return deepMergeRecords(acc, moduleContent);
    },
    {}
  );
}

const mergedModuleEn = mergeModuleTranslationsByLanguage(
  moduleTranslations,
  "en"
);
const mergedModuleEs = mergeModuleTranslationsByLanguage(
  moduleTranslations,
  "es"
);
