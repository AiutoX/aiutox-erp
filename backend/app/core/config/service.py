...
# Module -> exact key suffixes that must be encrypted at rest (Fernet, via
# encrypt_credentials/decrypt_credentials) in both system_configs and
# config_versions. Exact-match only, not prefix/glob, to keep behavior
# auditable (MSG-001 / SEC-001 retrofit).
SENSITIVE_KEYS: dict[str, set[str]] = {
    "notifications": {
...
def _is_sensitive(module: str, key: str) -> bool:
    return key in SENSITIVE_KEYS.get(module, set())
...
class ConfigService:
    ...
    def get(self, tenant_id: UUID, module: str, key: str, default: Any = None) -> Any:
        ...
        if config:
            value = config.value
            if _is_sensitive(module, key) and value:
                assert isinstance(value, str)
                value = decrypt_credentials(value, tenant_id)
            # Cache the value (plaintext — cache is not the at-rest boundary
            # this encryption protects; see decision log)
            if self.cache:
    ...
    def set(
        ...
        """
        # Validate schema if registered (validates the plaintext value)
        if not config_schema.validate(module, key, value):
            raise ValueError(
                f"Invalid value for {module}.{key}: value does not match schema"
            )

        # Encrypt sensitive values before persisting. `value` stays plaintext
        # for the cache, the audit log (redacted below), and the returned dict.
        sensitive = _is_sensitive(module, key)
        stored_value = value
        if sensitive and value:
            stored_value = encrypt_credentials(str(value), tenant_id)
        ...
        if is_update:
            ...
            config = self.repository.update(tenant_id, module, key, stored_value)
        else:
            config = self.repository.create(tenant_id, module, key, stored_value)

        # Create version record (encrypted snapshot for sensitive keys, so
        # version history never retains plaintext either)
        ...
        # Audit log — redact both sides for sensitive keys so audit_logs
        # never becomes a second plaintext-leak vector for the same secrets.
        action = "config.updated" if is_update else "config.created"
        details = {
            "module": module,
            "key": key,
            "old_value": REDACTED_PLACEHOLDER if sensitive else old_value,
            "new_value": REDACTED_PLACEHOLDER if sensitive else value,
        }
        create_audit_log_entry(
            ...
            resource_id=UUID(str(config.id)),
            details=details,
    ...
    def get_module_config(self, tenant_id: UUID, module: str) -> dict[str, Any]:
        ...
        for config in configs:
            value = config.value
            if _is_sensitive(module, config.key) and value:
                assert isinstance(value, str)
                value = decrypt_credentials(value, tenant_id)
    ...
    def delete(
        ...
        if self.use_versioning and self.version_repository and old_config:
            ...
            self.version_repository.create_version(
                config_id=UUID(str(old_config.id)),
                tenant_id=tenant_id,
                module=module,
                key=key,
                value=old_value,
        ...
        create_audit_log_entry(
            ...
            resource_type="config",
            resource_id=UUID(str(config_id)) if config_id else None,
            details=details,
            ip_address=ip_address,
            user_agent=user_agent,
        )
    ...
    def rollback_to_version(
        ...
        # target_version.value is stored ciphertext for sensitive keys
        # (post-MSG-001 snapshots). Decrypt to plaintext here so self.set()
        # below re-encrypts exactly once, avoiding double-encryption.
        target_value = target_version.value
        if _is_sensitive(module, key) and target_value:
            assert isinstance(target_value, str)
            try:
                target_value = decrypt_credentials(target_value, tenant_id)
            except ValueError:
                # Legacy pre-migration snapshot: already plaintext.
                pass
