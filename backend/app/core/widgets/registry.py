...
class WidgetRegistry:
    ...
    def get_available_widgets(
        ...
        and, when `user_permissions` is given, that the user is permitted to
        load — matching each widget's `data_endpoint` enforcement so this
        list never offers a widget the caller would then get a 403 loading.

        `user_permissions=None` applies no permission filtering (internal/
        system callers), the same convention reporting's per-dataset
        enforcement uses (see `column_rule_service.py`). `WidgetManifestOut`
        callers (the `/widgets/available` endpoint) always pass the real
        ...
        for module in self._modules:
            ...
            for manifest in manifests:
                ...
                if (
                    manifest.permission is not None
                    and user_permissions is not None
                    and not has_permission(user_permissions, manifest.permission)
                ):
                    continue
