...
export function initializeReactDevTools() {
  ...
  if (window.__REACT_DEVTOOLS_GLOBAL_HOOK__) {
    // DevTools is installed and available
    console.debug("[React DevTools] DevTools detected and ready");
  } else {
    // DevTools is not installed - show a helpful message only once
      ...
      if (hook.renderers) {
        ...
        for (const renderer of hook.renderers.values()) {
          if (
            ...
            // Valid renderer found
            console.debug(
              "[React DevTools] Renderer configured correctly"
            );
          }
        }
      }
    } catch (error) {
      // Silently ignore errors - DevTools may not be fully initialized yet
      console.debug("[React DevTools] Error configuring:", error);
    }
  }
}
