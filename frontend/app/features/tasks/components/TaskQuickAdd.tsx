...
import { dueDateToInputValue, dueDateInputValueToIso } from "~/lib/utils/taskDates";
...
import { createEvent, createReminder } from "~/features/calendar/api/calendar.api";
import { useAuthStore } from "~/stores/authStore";
import { showToast } from "~/components/common/Toast";
import { MultiSelect } from "~/components/ui/multi-select";
import { useTags } from "~/features/tags/hooks/useTags";
import { FileUploader } from "./FileUploader";
import { EntityCommentList } from "~/features/comments/components/EntityCommentList";
import type {
  EventCreate,
  EventReminderCreate,
  RecurrenceType,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "event") {
        ...
        try {
          ...
          if (error instanceof Error && error.message.includes("timeout")) {
            ...
            setFormError(
              "Error al crear evento. Por favor, verifica los datos e intenta nuevamente."
            );
          }
          return;
        }
      ...
      const payload: TaskCreate = {
        ...basePayload,
        due_date: dueDateInputValueToIso(
          formData.due_date ?? "",
          Boolean(formData.all_day)
        ),
        start_at: undefined,
        end_at: undefined,
        all_day: Boolean(formData.all_day),
      ...
      showToast(
        status === 403
          ...
          : t("tasks.createError") || "Error al crear la tarea",
        "error"
      );
    }
  };
      ...
      <DialogContent className="sm:max-w-125 max-h-[90vh] overflow-y-auto">
          ...
          <>
            ...
            >
              ...
              {isEventMode ? (
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="quick-due-date-all-day">
                      {t("calendar.events.allDay")}
                    </Label>
                    <Switch
                      id="quick-due-date-all-day"
                      checked={Boolean(formData.all_day)}
                      onCheckedChange={(checked) => {
                        const iso = dueDateInputValueToIso(
                          formData.due_date ?? "",
                          Boolean(formData.all_day)
                        );
                        setFormData({
                          ...formData,
                          all_day: checked,
                          due_date: dueDateToInputValue(iso, checked),
                        });
                      }}
                    />
                  </div>
              ...
              {mode === "task" && (
                <div className="pt-4 border-t">
                  <div className="text-xs text-muted-foreground text-center p-3 bg-muted/20 rounded">
                    {t("tasks.infoNote.addFilesAndComments") ||
                      "Para anexar comentarios y agregar archivos relevantes abra la tarea recién creada"}
                  </div>
                </div>
              )}
                ...
                <div className="border-t pt-4 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-2">
                    ...
                    {recurrenceType && recurrenceType !== "none" && (
                      <div>
                        <Label
                          htmlFor="quick-recurrence-interval"
                          className="text-sm text-muted-foreground"
                        >
                  ...
                  {recurrenceType && recurrenceType !== "none" && (
                    <div className="grid grid-cols-2 gap-4 mt-3">
                      ...
                      <div>
                        <Label
                          htmlFor="quick-recurrence-count"
                          className="text-sm text-muted-foreground"
                        >
              ...
              {isEventMode && (
                <div className="border-t pt-4 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label
                        htmlFor="quick-reminder-minutes"
                        className="text-sm text-muted-foreground"
                      >
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="0">
                            {t("calendar.reminders.at_start_time") ||
                              "Al inicio"}
                          </SelectItem>
                          <SelectItem value="5">
                            5{" "}
                          ...
                          <SelectItem value="60">
                            1{" "}
                            {t("calendar.reminders.hours_before") ||
                              "hora antes"}
                          </SelectItem>
                          ...
                          <SelectItem value="1440">
                            1{" "}
                            {t("calendar.reminders.days_before") || "día antes"}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
        ...
        {createdTaskId && mode === "event" && (
          <div className="pt-4 border-t text-center text-muted-foreground">
            <p>
              Los archivos y comentarios para eventos estarán disponibles
              próximamente.
            </p>
          </div>
        )}
