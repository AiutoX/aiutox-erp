...
export type BadgeUpdate = Partial<BadgeCreate>;

export async function createBadge(
  data: BadgeCreate
): Promise<StandardResponse<Badge>> {
  const response = await apiClient.post<StandardResponse<Badge>>(
    "/gamification/badges",
    data
  );
  return response.data;
...
export async function updateBadge(
  ...
  const response = await apiClient.put<StandardResponse<Badge>>(
...
export async function deactivateBadge(badgeId: string): Promise<void> {
  await apiClient.delete(`/gamification/badges/${badgeId}`);
}
