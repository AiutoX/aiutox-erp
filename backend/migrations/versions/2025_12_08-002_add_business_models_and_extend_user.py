...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_organizations_tax_id"), "organizations", ["tax_id"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        "ix_organizations_tenant_type",
        "organizations",
        ["tenant_id", "organization_type"],
        unique=False,
    )
    op.create_unique_constraint(
        "uq_organizations_tenant_tax_id", "organizations", ["tenant_id", "tax_id"]
    )
    ...
    op.create_index(
        "ix_contact_methods_entity",
        "contact_methods",
        ["entity_type", "entity_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_contact_methods_entity_type"),
        "contact_methods",
        ["entity_type"],
        unique=False,
    )
    op.create_index(
        op.f("ix_contact_methods_entity_id"),
        "contact_methods",
        ["entity_id"],
        unique=False,
    )
    ...
    op.create_index(
        "ix_person_identifications_entity",
        "person_identifications",
        ["entity_type", "entity_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_person_identifications_entity_type"),
        "person_identifications",
        ["entity_type"],
        unique=False,
    )
    op.create_index(
        op.f("ix_person_identifications_entity_id"),
        "person_identifications",
        ["entity_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_person_identifications_document_number"),
        "person_identifications",
        ["document_number"],
        unique=False,
    )
    ...
    op.add_column(
        "users",
        sa.Column(
            "preferred_language", sa.String(10), server_default="es", nullable=False
        ),
    )
    ...
    op.add_column(
        "users", sa.Column("last_login_at", sa.TIMESTAMP(timezone=True), nullable=True)
    )
    op.add_column(
        "users",
...
def downgrade() -> None:
    ...
    op.drop_index(
        op.f("ix_person_identifications_document_number"),
        table_name="person_identifications",
    )
    op.drop_index(
        op.f("ix_person_identifications_entity_id"), table_name="person_identifications"
    )
    op.drop_index(
        op.f("ix_person_identifications_entity_type"),
        table_name="person_identifications",
    ...
    op.drop_index(
        "ix_person_identifications_entity", table_name="person_identifications"
    )
    op.drop_table("person_identifications")

    # Drop contact_methods table
    op.drop_constraint(
        "uq_contact_methods_entity_type_value", "contact_methods", type_="unique"
    )
    ...
    op.drop_constraint(
        "uq_organizations_tenant_tax_id", "organizations", type_="unique"
    )
    op.drop_index("ix_organizations_tenant_type", table_name="organizations")
    op.drop_index(
        op.f("ix_organizations_organization_type"), table_name="organizations"
    )
