...
@router.post(
    ...
    summary="Upload file",
    description="Upload a new file. Requires files.write permission.",
)
async def upload_file(
    ...
    if permissions:
        try:
            ...
            raise APIException(
                code="FILE_INVALID_PERMISSIONS_JSON",
...
@router.get(
    ...
    description="Get file information by ID. Requires files.read permission and specific file view permission.",
)
async def get_file_info(
    ...
    file_tags = []
    try:
        tags = service.get_file_tags(UUID(str(file.id)), current_user.tenant_id)
...
@router.get(
    ...
    description="Download file content. Requires files.read permission and specific file download permission.",
    responses={
        200: {
...
async def download_file(
    ...
    try:
        content, file = await service.download_file(
...
@router.get(
    ...
    description="Get a preview or thumbnail of an image file. Requires files.read permission and specific file view permission.",
    responses={
...
@router.put(
    ...
    status_code=status.HTTP_200_OK,
    summary="Update file",
    description="Update file information. Requires files.write permission and specific file edit permission or ownership.",
)
...
@router.delete(
    ...
    summary="Delete file",
    description="Delete a file (soft delete). Requires files.delete permission and specific file delete permission or ownership.",
)
...
@router.post(
    ...
    description="Restore a soft-deleted file. Requires files.admin permission.",
)
async def restore_file(
    file_id: Annotated[UUID, Path(..., description="File ID")],
    current_user: Annotated[User, Depends(require_permission("files.admin"))],
    service: Annotated[FileService, Depends(get_file_service)],
) -> StandardResponse[FileResponse]:
...
@router.get(
    ...
    description="List all versions of a file. Requires files.read permission and specific file view permission.",
)
...
@router.post(
    ...
    description="Create a new version of a file. Requires files.write permission and specific file edit permission or ownership.",
)
...
@router.get(
    ...
    description="Download a specific file version. Requires files.read permission and specific file download permission.",
)
...
@router.post(
    ...
    description="Restore a specific file version by creating a new version with its content. Requires files.admin permission and specific file edit permission or ownership.",
)
...
@router.get(
    ...
    description="Get all permissions for a file. Requires files.read permission and specific file view permission or ownership.",
)
async def get_file_permissions(
    ...
    return StandardListResponse(
        ...
        meta=PaginationMeta(
            total=count, page=1, page_size=max(1, count), total_pages=1
        ),
...
@router.put(
    ...
    description="Update file permissions. Requires files.admin permission and specific file edit permission or ownership.",
)
...
@router.post(
    ...
    description="Add tags to a file. Requires files.write permission and specific file edit permission or ownership.",
)
...
@router.delete(
    ...
    description="Remove a tag from a file. Requires files.delete permission and specific file edit permission or ownership.",
)
...
@router.get(
    ...
    description="Get all tags for a file. Requires files.read permission and specific file view permission.",
)
...
@router.get(
    ...
    description="Get raw content of a text file. Only works for text files (text/*, application/json, etc.). Requires files.read permission and specific file view permission.",
    responses={
        200: {
            "description": "File content",
            "content": {
...
async def get_file_content(
    ...
    try:
        # Download file content
        content, _ = await service.download_file(
            file_id, current_user.tenant_id, current_user.id
        )
...
@router.get(
    ...
    status_code=status.HTTP_200_OK,
    summary="List files",
    description="List files that the user can view. Requires files.read permission and filters by specific file permissions.",
)
async def list_files(
        ...
        for file in all_files:
            try:
                if service.check_permissions(
                    UUID(str(file.id)), current_user.id, current_user.tenant_id, "view"
                ):
                    files.append(file)
            except FileNotFoundError:
                continue
    ...
    for file in files:
        ...
        file_tags = []
        try:
            file_tags_list: list[Any] = service.get_file_tags(
                UUID(str(file.id)), current_user.tenant_id
            )
            file_tags = [TagResponse.model_validate(tag) for tag in file_tags_list]
        except FileNotFoundError:
            pass  # File not found, skip tags
        except Exception as e:
            logger.warning(f"Error getting tags for file {file.id}: {e}")
