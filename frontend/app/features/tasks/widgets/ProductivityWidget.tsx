...
export function ProductivityWidget({
  ...
  const formatValue = (value: number | string, format: string) => {
    ...
    switch (format) {
      ...
      case "time":
        ...
        return typeof value === "number" ? `${safeValue}h` : value;
      case "number":
      default:
  ...
  const getProgressValue = (kpi: KPIData) => {
    const rawValue = Number(kpi.value);
    if (kpi.format === "percentage") {
      return Math.min(100, Math.max(0, Number.isFinite(rawValue) ? rawValue : 0));
    }
              ...
              <div key={index} className="p-4 bg-muted/30 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <div
                      className={`p-1 rounded-lg ${kpi.color.replace("text", "bg")} bg-opacity-10`}
                    >
                      {kpi.icon}
                    </div>
                    <div>
